...
/*
 * academic_program_type(학술 활동 유형) — study/project 구분을 코드가 아니라 데이터로 두는
 * 기준 데이터 (#130).
 *
 * enum이 아니라 테이블인 것은 의도된 것이다. 세미나·특강·대회 등으로 유형이 늘어날 수 있다는
 * 방침(학술관리_기능범위.md §2)에 따라, 배포 없이 시드 추가만으로 확장 가능해야 한다.
 *
 * PK가 IDENTITY가 아니라 코드 문자열인 것은 academic_program.academic_program_type_cd가 이
...
@Table(name = "academic_program_type")
@Getter
...
public class AcademicProgramTypeEntity {
    ...
    @Id
    @Column(name = "academic_program_type_cd", length = 30)
    private String code;
