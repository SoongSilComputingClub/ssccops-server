...
public enum OperationErrorCode implements ErrorCode {
    ...
    /*
     * 409 — 이미 체크된 완료 점검 항목을 지우려 했을 때 (#307 · ssccops#255 결정).
     *
     * **여기서만 코드를 갈랐다.** 이 도메인의 관행은 전용 코드를 늘리지 않고
     * TRANSITION_NOT_ALLOWED를 재사용하는 것이고(requireChecklistEditable·requireVotable),
     * 항목 편집이 검토 단계에서 막히는 거절은 그 관행을 그대로 따른다. 이 하나만 나눈 이유는
     * **두 거절의 해소 방법이 다르기 때문**이다 — 상태 잠금은 되돌리려면 반려로 진행에
     * 되돌아가야 하고 목록 전체에 동시에 걸리지만, 이쪽은 체크를 해제하면 곧바로 지울 수
     * 있고 항목 하나에만 걸린다. 한 코드로 묶으면 화면이 두 안내를 구별하려고 message
     * 문자열로 분기하게 되는데, 메시지는 계약이 아니라 언제든 다듬는 문구다.
     *
     * 정의서 03_오류_코드에 코드를 늘리는 비용은 있다. 그 비용이 가장 싼 때가 같은 Story의
     * 웹 작업이 나란히 도는 지금이며, PR 본문에 코드와 필드 이름을 함께 못 박는다.
     *
     * 체크된 항목을 지울 수 없게 한 근거 자체는 "해당 없음으로 지우기"와 "안 하고 지우기"가
     * 화면에서 구별되지 않는다는 것이다 — 체크 안 된 것만 지울 수 있으면 지운다는 행위 자체가
     * "이번 건엔 해당 없다"는 선언이 된다.
     */
    CHECKLIST_ITEM_COMPLETED(
            HttpStatus.CONFLICT, "CHECKLIST_ITEM_COMPLETED", "이미 체크한 항목은 삭제할 수 없습니다."),
