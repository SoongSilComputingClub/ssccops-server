...
public final class AssistantGoldenSet {
    ...
    /*
     * 답해야 하는 질문과 **그 답이 기대는 인용 표기**(`RetrievedChunk.marker()`).
     *
     * ⚠️ **#447로 이 표기를 쓰는 주체가 바뀌었다** — 예전에는 모델이 대괄호에 옮겨 적는 문자열
     * 이었고 지금은 **서버가 청크에서 만들어 응답에 싣는 값**이다
     * (`AssistantCitationResponse.marker`). 모델이 쓰는 것은 발췌 번호뿐이다. 정답지를 그대로
     * 둔 것은 이 표가 답하는 물음이 «어느 근거를 기대는가»이지 «모델이 무엇을 타이핑했는가»가
     * 아니기 때문이고, 그래서 이 표가 여전히 인용 정확도의 정답지다.
     *
