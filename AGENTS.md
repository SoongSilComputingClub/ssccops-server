...
- **도구는 38종이고 여섯 클래스다** — `OperationTools`(1차 읽기 7 + 전이·체크 2 · #385) · **`WorkTools`**(업무·하위 업무 쓰기 8) · **`MeetingTools`**(회의 쓰기 4 + 승인함·대시보드·유형 읽기 3) · **`EventTools`**(W2 · #494 — `list_events`·`get_event`·`create_event`·`update_event`(`EventPatch` 읽고-합치기 · 서버가 PUT이라 `McpRestClient.put`)·`change_event_status`(PUBLISH·RETRACT·ARCHIVE·REPUBLISH) — 복제·삭제·이미지·공유 도구는 없다) · **`FormTools`**(W2 — `list_forms`(`receiptStatuses` 다중)·`get_form`(문항 포함)·`change_form_status`(OPEN·CLOSE) — **문항을 만들거나 고치는 도구는 없다**: 문항 JSON을 자연어로 조립하면 편집기가 못 여는 폼이 생긴다, 응답 도구도 없다) · **`ContentTools`**(콘텐츠 6 — `create_page`·`update_page`·`create_post`·`update_post`·`publish_content`(kind=page|post + publish 불리언, 출력은 kind에 따라 `ContentPageResponse`/`ContentPostResponse`)·`request_content_image_upload`(presigned PUT + `fileId` — 바이트는 스크립트가 올린다) · ssccops#381 · ADR-0038 · 규칙은 `domain/content/AGENTS.md`). 뒤의 둘은 W1(ssccops#365 · Story)이며 어드민 기능 전체로 넓히는 파도의 첫 번째다 — 남은 파도는 학술 → 폼 문항 편집 → 회원(+소프트 삭제)이고(행사·폼 조회/상태는 W2로 끝) 회원명·삭제 범위는 [ADR-0037](https://github.com/SoongSilComputingClub/ssccops/blob/develop/docs/decisions/0037-mcp-tool-output-carries-member-names-and-soft-delete-only.md)이 정했다.
