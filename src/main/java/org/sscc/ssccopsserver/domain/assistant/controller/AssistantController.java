...
public class AssistantController {
     ...
     *
     * 같은 사실을 두 벌 두면 «모델은 40초를 기다리는데 응답은 10초에 닫히는» 조합이 만들어지고,
    ...
    @Operation(
            ...
            description =
                    "질문 하나에 답하고 근거가 된 인용을 함께 내린다. **화면은 보통 /queries/stream 을 쓴다** —"
                            ...
                            + " 돌려주며 citations는 빈 배열이다(null이 아니다). 모델이 발췌를 받고도"
                            + " 근거가 없다고 판단한 답도 **같은 모양으로** 나간다 — 그 답에 인용이"
                            + " 몇 개 달렸든 answered=false이고 citations는 비어 있다. **모델은 조 번호가 아니라"
    ...
    @Operation(
            ...
            description =
                    "같은 질문에 **답을 흘려보내며** 답한다(text/event-stream). 이벤트는 셋이다 —"
                            ...
                            + " 안내 문구를 싣는다. 모델이 발췌를 받고도 근거가 없다고 판단한 경우도"
                            + " 같은 모양이다(그 판단은 화면에 글자가 닿기 전에 끝난다). 흘려보내기 시작한 뒤의 모델 실패는 상태 코드를 바꿀 수"
     ...
     */
    @Operation(
            summary = "규정 도우미 추천 질문 · 코퍼스 상태",
            description =
                    "지금 코퍼스가 답할 수 있는 질문을 최대 3개 내리고, **화면의 빈 상태가 무엇을 말해야"
                            + " 하는지**를 corpusState로 함께 내린다. 값은 셋이다 — EMPTY(문서가 한"
                            + " 건도 없다. 새 환경의 기본 상태다) · NONE_EFFECTIVE(문서는 있는데"
                            + " **검색 대상이 하나도 없다** — 업로드는 언제나 DRAFT로 들어오고"
                            + " 「시행」을 눌러야 검색 대상이 되므로 첫 업로드 직후가 이 상태다. 색인"
                            + " 중·색인 실패·내려둔 문서뿐인 경우도 여기 속한다) · READY(검색 대상이"
                            + " 하나라도 있다). **questions는 READY일 때만 채워지고 나머지 둘에서는 빈"
                            + " 배열이다** — 참고용이어도 누르면 실제 질의가 나가므로 답할 수 없는"
                            + " 질문을 권하지 않는다. **서버가 내리는 것은 상태이고 문구가 아니다**:"
                            + " 같은 NONE_EFFECTIVE에서도 RAG_DOCUMENT_MANAGE를 가진 사람에게는"
                            + " «시행을 누르세요»이고 그렇지 않은 사람에게는 «운영진에게 문의»인데, 그"
                            + " 갈림의 재료(capabilities)는 화면이 이미 들고 있다. **인증만 요구하는"
                            + " 것이 이 값을 여기 두는 이유다** — 문서 목록·요약은"
                            + " RAG_DOCUMENT_MANAGE 뒤에 있어 일반 회원은 403이고, 도우미 패널은"
                            + " 어디서나 열린다.")
