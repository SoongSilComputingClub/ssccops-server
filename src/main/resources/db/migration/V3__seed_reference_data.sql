-- ============================================================================
-- V2 — 기준 코드·기준 데이터 시드 (data.sql에서 옮겨 왔다)
-- ============================================================================
-- ⚠ **아직 돌지 않는다.** `spring.flyway.enabled`가 모든 프로필에서 false이고, 시드는 여전히
-- `data.sql`이 넣고 있다. baseline을 실물로 교체하고 Flyway를 켜는 시점에 `data.sql`을 지운다
-- (두 벌이 동시에 도는 상태로 두지 않는다 — 지금은 이 파일이 잠들어 있어 겹치지 않는다).
--
-- **왜 versioned(V)이고 repeatable(R)이 아닌가.**
-- `data.sql`은 `spring.sql.init.mode=always`라 매 기동마다 돌았고, 그래서 값을 하나 더하면
-- 다음 배포에 조용히 반영됐다. 그 편리함이 곧 #241이 터진 자리다 — **무엇이 언제 들어갔는지
-- 남지 않는다.** 버전 마이그레이션은 한 번만 돌고 이력이 남으며, 값을 더할 때 새 마이그레이션
-- 파일을 만들게 강제한다. 그 파일이 곧 "언제 무엇을 넣었나"의 답이다.
--
-- `WHERE NOT EXISTS`는 그대로 둔다. 버전 마이그레이션이라 한 번만 돌지만, baseline이 이미
-- 시드된 prod 덤프이므로 **이 파일이 처음 도는 DB에도 행이 이미 있다.** 가드가 없으면 그 자리에서
-- 중복 키로 깨진다. 가드의 원래 뜻(운영진이 화면에서 고친 값을 배포가 되돌리지 않는다)도 함께 산다 —
-- **값을 고칠 때 UPDATE로 덮어쓰지 말 것.**
--
...
-- 공유하는 계약이라 SQL 문자열로 두면 선언과 시드가 두 벌이 되기 때문이다. 폼이 안 보인다고
-- 이 파일에서 찾지 말 것 — 여기 없는 것이 정상이다.
--
...
--
-- 승인·투표 자격은 역할 행이 아니라 역할↔권한 매핑(role_authrt_rel)이 갖는다(#123).
-- 직위 코드 컬럼(role_pstn_cd, #118)은 자격이 권한 시스템으로 통합되며 삭제됐다 — 부서별
-- 국장(홍보국장 …)을 만들 때는 역할별 권한 화면에서 SUB_WORK_APPROVE_DIRECTOR를 부여한다.
...
--
-- 최고관리자는 SUPER를 통해 결재·투표 권한(APPROVAL 그룹)까지 자동 보유한다(#123).
-- #118의 '시스템 관리와 조직 결재를 섞지 않는다'(직위 코드 NULL)는 유지하지 않는다(ssccops#108) —
-- 분리가 다시 필요하면 역할 구성으로 해결한다.
INSERT INTO role (indct_seqno, role_nm, role_clsf_cd)
