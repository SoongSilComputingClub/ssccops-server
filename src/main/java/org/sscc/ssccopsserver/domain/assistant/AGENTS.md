...
- **응답 상한은 새 손잡이가 아니라 `ssccops.assistant.gemini.call-timeout` + 10초다.** 같은 사실을
  두 벌 두면 «모델은 20초를 기다리는데 응답은 10초에 닫히는» 조합이 만들어지고, 그 고장은 «긴
  답변에서만 글자가 나오다 끊기는» 모양으로 나타난다.
- ⚠️ **아직 실측하지 않은 것** — OkHttp `callTimeout`은 **본문을 다 읽을 때까지**라 긴 답변이
  20초에 잘릴 수 있다. 그때 실패의 모양은 통째로 503이 아니라 **글자가 나오다 끊기고 `error`
  이벤트가 오는 것**이다. 실제 키로 재기 전까지는 그렇게 될 수 있다고만 적어 둔다.
