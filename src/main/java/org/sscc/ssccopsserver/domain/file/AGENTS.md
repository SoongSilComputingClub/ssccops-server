...
- `FileReferenceEntity`(`file_rfrnc`) · `FileReferenceService` · `FilePresigner` · `FileEraser` · `FileCopier` · `FileUploader` · `FileDownloader` · 코드 `ImageFileType`·`FileTargetType`. 컨트롤러가 없다 — 이 도메인은 다른 도메인의 서비스가 부르는 쪽이다.
- 하지 않는 것: 접근 제어 · 읽기 주소 조립 · 대상당 건수 제한(아래 «올라오지 않는 것» 항목).

## 규칙

- `domain/file` — **파일이 버킷의 어디에 있는가**를 아는 유일한 도메인 (#220). 여기 있는 것은 여섯뿐이다: `file_rfrnc` 행(`entity/FileReferenceEntity` · `service/FileReferenceService`), 서명(`service/FilePresigner`), 업로드 허용 형식(`code/ImageFileType`), 삭제(`service/FileEraser` · #234), 복사(`service/FileCopier` · ssccops#198), 바이트를 올리고 내려받는 것(`service/FileUploader` · `service/FileDownloader` · #399·#400).
  ...
  - **`FileDownloader`는 그 반대 방향이고 쓰는 자리는 색인 워커 하나다** (#400). 서명 URL을 내주고 워커가 HTTP로 받아 오는 길을 택하지 않은 것은, 우리 서버가 우리 버킷에서 읽으려고 공개 주소를 한 번 만들었다 버리는 일이고 실패가 SDK 예외가 아니라 HTTP 상태로 바뀌어 원인이 한 겹 멀어지기 때문이다. **바이트를 통째로 메모리에 올리는 것이 여기서 괜찮은 이유**는 `FileUploader`와 같다 — 파싱하려면 어차피 내용을 읽어야 하고, 업로드 상한 10MB와 색인 워커의 동시 실행 1건이 한 번에 붙드는 양을 끊는다. 그래서 `FileCopier`가 서버 측 `CopyObject`를 쓰는 근거(#107)는 그쪽에 그대로 남는다. **없는 키는 예외로 올린다** — 삼켜 빈 배열을 돌려주면 «내용이 없는 문서»와 «원본이 사라진 문서»가 색인 실패 사유에서 같은 모양이 된다.
  ...
  - **`file_rfrnc`는 학술 전용이 아니다.** #137이 회차 출석 인증사진용으로 만들었고 `sesn_id` NOT NULL + UNIQUE가 도메인을 가르고 있었는데, 파일이 붙는 자리가 늘 때마다 테이블을 새로 만들지 않으려고 소유자를 **`trgt_se_cd`(대상_구분_코드) + `trgt_id`(대상_ID)** 두 값으로 열었다. 전제는 #200이 이미 깔아 두었다 — 저장 값이 조립된 URL이 아니라 **오브젝트 키**라 도메인 중립이다. 대상을 늘리는 일은 `FileTargetType`에 한 줄과 표준코드 시트에 한 줄이며, 지금 코드값은 `SESSION`(학술 회차 인증사진) · `RAG_DOCUMENT`(규정 문서 원본, #402) · `CONTENT_POST`(공개 사이트 포스트 갤러리, ssccops#381) 셋이다. **`CONTENT_POST`는 대상당 여러 건이 정상인 첫 대상**이라 `FileReferenceRepository.findAllByTargetTypeAndTargetIdOrderByIdAsc`·`findByIdAndTargetTypeAndTargetId`와 `FileReferenceService.findAllByTarget`·`findOneOfTarget`·`add`·`deleteOneOfTarget`이 그때 생겼다 — 단건 `findByTarget`을 그 대상에 쓰면 둘째 행부터 조용히 사라진다. CHECK 제약은 V15가 넓혔고 `idx_file_rfrnc_trgt`(대상 구분·대상 id)가 함께 붙었다.
  - **FK를 걸지 않는다.** 대상 테이블이 여럿이라 걸 수 없고, 배타적 FK(`sesn_id`·`event_id`·… + CHECK)로 가면 대상이 하나 늘 때마다 스키마와 데이터사전이 바뀐다(`ddl-auto: update`가 nullable 전환을 반영하지 않아 그때마다 dev·prod 수동 ALTER가 붙는다). 대가는 고아 행이고 **정리는 각 도메인의 책임**이다 — 애초에 이 행은 실물을 가리킨다는 보장이 없다(서버가 PUT을 관측하지 않는다). ⚠️ **(2026-09-24 · #558) 이 줄은 `ddl-auto: update`이던 시절의 기록이다** — 지금 `dev`·`prod`는 `validate`이고 스키마는 마이그레이션 파일이 만든다. 지나간 작업의 흔적이라 지우지 않는다.
  ...
  - **첨부 메타 열은 운영 건 첨부만 채운다** (#493 · V18 `orgnl_file_nm`·`file_size`·`rgtr_mbr_id`·`crt_dt` — 전부 NULL 허용). 사용자에게 «무슨 파일인가»를 보여야 하는 첫 대상이라 생겼고, 회차 사진·규정 문서·갤러리는 NULL이며 읽지 않는다. `rgtr_mbr_id`에 FK를 걸지 않는 것은 표시용이라서(회원이 지워져도 첨부는 남는다). 허용 형식은 `AttachmentFileType`(이미지 목록 `ImageFileType`과 별개 — 이유가 다르다). `FilePresigner.presignGet(key, downloadFileName)`은 `response-content-disposition`(RFC 5987)으로 원본 이름을 붙인다.
...
- 콘텐츠: 포스트 갤러리(`FileTargetType.CONTENT_POST` · 키 접두사 `content-posts/`, ssccops#381). 발급형 업로드인데도 **행을 남기는** 이유(표지 FK · «몇 장인가»를 서버가 답한다)와 «누가 볼 수 있는가»(게시본이면 익명)는 `content`가 답한다 — 이 도메인은 여전히 묻지 않는다. 키 조립은 `content`의 `ContentImageLocation` 한 곳.
