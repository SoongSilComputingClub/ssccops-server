...
public record RetrievedChunk(Document chunk, SearchableDocument source) {
    ...
    /**
     * 이 발췌를 가리키는 <b>짧은 표기</b> — `제7조` · `부칙 제3조` · `p.12` · 문서명.
     *
     * <p><b>모델은 이 문자열을 쓰지 않는다</b>(#447). 예전에는 프롬프트가 이것을 적어 주고 모델이 대괄호로 옮겨 쓰게 했는데, 지금 모델이 쓰는 것은 발췌
     * 번호(`[3]`)뿐이고 <b>이 표기는 서버가 그 번호로 찾아 붙인다</b>({@code CitationVerifier} → {@code
     * AssistantCitationResponse.marker}). 그래서 <b>틀린 조 번호가 발생할 수 없다</b> — 옮겨 적다 틀릴 자리가 없다.
     *
     * <p>화면은 본문의 `[3]`을 그대로 둘 수도, 이 표기로 갈아 그릴 수도 있다. 서버는 둘 다 내리고 고르지 않는다.
     *
