...
/*
 * 계획(crclm_artcl) + 실적(sesn) 조인 한 줄 (#134, 학술관리_API설계.md §3.3). 활동 상세
 * 화면의 "커리큘럼 대비 진행" 표가 이 배열 하나로 그려진다.
 *
 * sesnSttsCd는 실적 행이 없어도 NOT_SUBMITTED 문자열로 채워 내려간다(설계 결정 #1) —
 * 클라이언트가 null 분기를 갖지 않게 하려는 것이며, 역할·상태를 웹에서 재계산하지 않는다는
 * 원칙의 확장이다. 반대로 sessionId·actlYmd·prgrsCn은 실적이 있을 때만 채운다(없는 것을 있는 척
 * 하지 않는다).
 *
...
public record CurriculumItemWithSessionResponse(
    ...
    /*
     * 실적이 없는 계획 한 줄. 상태는 저장된 값이 아니라 "sesn 행이 없다"는 사실에서 합성된다
