...
- **도구는 54종이고 일곱 클래스다** — `OperationTools`(1차 읽기 7 + 전이·체크 2 · #385) · **`WorkTools`**(업무·하위 업무 쓰기 8) · **`MeetingTools`**(회의 쓰기 4 + 승인함·대시보드·유형 읽기 3) · **`EventTools`**(W2 · #494 — `list_events`·`get_event`·`create_event`·`update_event`(`EventPatch` 읽고-합치기 · 서버가 PUT이라 `McpRestClient.put`)·`change_event_status`(PUBLISH·RETRACT·ARCHIVE·REPUBLISH) — 복제·삭제·이미지·공유 도구는 없다) · **`FormTools`**(W2 · #494 + W4 · #587 — `list_forms`(`receiptStatuses` 다중)·`get_form`(문항 포함)·`change_form_status`(OPEN·CLOSE)·`list_form_labels`·`assign_form_labels`(**전체 교체** — 하나를 더하려면 지금 걸린 것을 함께 보낸다)·`list_form_responses`·`review_form_response`(ACCEPTED·CHANGES_REQUESTED·REJECTED · 수정요청·반려는 의견 필수 · 승인·반려는 종결이라 되돌릴 수 없다) — **문항을 만들거나 고치는 도구와 응답 상세 도구는 없다**, 아래) · **`AcademicTools`**(W3 · #567 — 활동·회차·기획안 검토·모집 선발·출석 정정 12 · 만들기 도구는 없다: 활동은 기획안 승인 이관으로만 생긴다) · **`ContentTools`**(콘텐츠 6 — `create_page`·`update_page`·`create_post`·`update_post`·`publish_content`(kind=page|post + publish 불리언, 출력은 kind에 따라 `ContentPageResponse`/`ContentPostResponse`)·`request_content_image_upload`(presigned PUT + `fileId` — 바이트는 스크립트가 올린다) · ssccops#381 · ADR-0038 · 규칙은 `domain/content/AGENTS.md`). 어드민 기능 전체로 넓히는 파도는 ssccops#365이고 **W1 운영 → W2 행사·폼 → W3 학술 → W4 폼·응답까지 끝났다 — 남은 것은 W5 회원(+소프트 삭제)**이다. 회원명·삭제 범위는 [ADR-0037](https://github.com/SoongSilComputingClub/ssccops/blob/develop/docs/decisions/0037-mcp-tool-output-carries-member-names-and-soft-delete-only.md)이 정했고 폼 문항 편집은 그 Story의 범위 밖이다.
  ...
  - **삭제 도구는 아직 없다** — 소프트 삭제만 열기로 했고(ADR-0037) W5에서 낸다. 하드 삭제는 어느 파도에서도 열지 않는다. `advertisesEveryOperationTool`이 `delete_`로 시작하는 도구가 없음을 지킨다.
