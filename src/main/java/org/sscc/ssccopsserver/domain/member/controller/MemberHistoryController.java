 ...
 * (네 이력 테이블)이라는 것이고, 다른 하나는 MemberController가 이미 가입·목록·상세·수정을
...
public class MemberHistoryController {
    ...
    /*
     * page 봉투를 싣지 않는다 (AP-11). 네 출처를 합치므로 단일 컬럼 커서가 성립하지 않고 한
    ...
    @Operation(
            ...
            description =
                    "등급(mbr_grd_hstry)·상태(mbr_stts_hstry)·역할(mbr_role_rel)·회원 정보"
                            + "(mbr_chg_hstry) 이력을 하나의 타임라인으로 합쳐 발생 시각 역순으로 내린다."
                            + " type으로 출처를 고를 수 있고(GRADE·STATUS·ROLE·PROFILE, 복수 허용)"
                            + " 생략하면 전부다. 역할은 한 배정이 부여(ROLE_ASSIGNED)와 종료(ROLE_ENDED)"
                            + " 두 줄로 나오며, mbr_role_rel에 변경자 컬럼이 없어 changedBy는 항상 null이다."
                            + " 회원 정보 변경(PROFILE)은 바뀐 항목마다 한 줄이며 어느 항목인지는"
                            + " changeField(코드)·changeFieldName(표시명)이 답한다 — 값은 previousName·"
                            + "newName에 담기고 previousCode·newCode·appliedDate·changeReason은 null이다."
                            + " 반대로 등급·상태·역할 줄에서는 changeField·changeFieldName이 null이다."
                            + " 이력이 없는 회원은 빈 배열이고, 없는 회원은 404 NOT_FOUND, 알 수 없는 type은"
                            + " 400 VALIDATION_FAILED다. 페이징을 두지 않으므로 응답은 배열이다.")
    @GetMapping
