 ...
 *
 * **aprvSeCd는 SESSION·COMPLETION 둘뿐이다.** 기획안 승인(구 PROPOSAL 지점)은 2026-08-24
...
public record AcademicProgramApprovalCondition(
    ...
    private AcademicProgramApprovalPoint toPoint() {
        if (aprvSeCd == null || aprvSeCd.isBlank()) {
            return null;
        }
        try {
            return AcademicProgramApprovalPoint.valueOf(aprvSeCd.strip());
