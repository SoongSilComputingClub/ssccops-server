...
public enum OperationErrorCode implements ErrorCode {
    ...
    /*
     * 404 — 공유 링크 미리보기가 열리지 않는 모든 경우 (ssccops#200 · ADR-0016).
     *
     * **없는 토큰·폐기된 토큰·지워진 운영 건을 하나로 묶는다.** 나누면 그 차이가 곧 "그
     * 토큰은 있었다(또는 폐기됐다)"는 정보가 되고, 이 코드는 익명에게 나가므로 그 정보는
     * 아무 자격 없는 사람에게 간다. 소프트 삭제를 404로 묶은 다른 조회들과 같은 태도다.
     */
