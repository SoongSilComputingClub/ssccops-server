...
/*
 * atndc(회차별 출석) — 회차 하나에 대한 참가자별 참석 여부 (#135, 학술관리_데이터모델.md
 * §2). 회차 기록 화면이 진행 내용과 출석 체크를 한 화면·단일 제출 버튼으로 받으므로, 이 행들은
 * sesn과 같은 트랜잭션에서 함께 만들어진다.
 *
 ...
 *
 * (sesn_id, event_ptcp_id) UNIQUE는 ERD에 없지만 여기서 건다 — 한 회차에 같은 참가자가 두
...
@Table(
        ...
        uniqueConstraints =
                @UniqueConstraint(
                        name = "uk_atndc_sesn_ptcp",
                        columnNames = {"sesn_id", "event_ptcp_id"}))
@Getter
...
public class AttendanceEntity {
    ...
    @Column(name = "atnd_yn", nullable = false)
