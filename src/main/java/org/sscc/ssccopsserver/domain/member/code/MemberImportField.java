 ...
 *
 * **그 규약의 예외가 하나 있다 — CLUB_JOIN_PERIOD(`clbJoinYm`)** (#205). 저장은 두 컬럼
 * (clb_join_yr_no·clb_join_mm_no)으로 갈라지는데 명부 엑셀은 가입 시기를 한 칸에 적기 때문이다.
 * 매핑 대상을 둘로 나누면 운영자가 128행짜리 엑셀을 미리 쪼개야 하고 그 과정에서 틀린다.
 *
...
public enum MemberImportField {
    ...
    /*
     * 기수. 미입력은 0(미배정)이며 **학번으로 추정하지 않는다** (BR-M43) — 추정한 기수는 나중에
     * 사실과 구별되지 않는다.
     *
     * **동아리 가입 연도로도 자동으로 채우지 않는다** (#205). 연도는 학번과 달리 기수의 정의
     * 그 자체지만, 이관은 사람이 한 행씩 확인하지 않는 대량 경로라 여기서 계산하면 128명의
     * 기수가 검증 없이 들어간다 — 그것이 BR-M43이 경고한 상황 그대로다. 자동 제안은 운영진이
     * 한 명씩 보는 편집 화면에서만 동작한다(GET /v1/members/generation).
     */
    ...
    /*
     * 동아리 가입 시기 (#205). 명부의 '가입일' 칸이 여기로 온다 — 이관 대상은 아직 전산 시스템에
     * 가입한 적이 없는 사람이라, 그 칸에 적힌 것은 운영진이 기록한 **동아리 입부일**이다. 전산
     * 가입일은 언제나 이관을 실행한 날이므로 명부에서 받을 값이 아니고, 그래서 옛 joinYmd 매핑을
     * 없앴다 — 지금까지 그 값이 join_ymd로 들어가 두 뜻이 섞였다.
     *
     * 별칭을 그대로 물려받고 입부 어휘를 더한다. 미입력은 **비운다** — 옛 매핑은 이관일로
     * 채웠지만, 모르는 것을 모른다고 두는 것이 이 컬럼의 존재 이유다.
     */
    CLUB_JOIN_PERIOD(
            "clbJoinYm",
            "동아리 가입 시기",
            false,
            List.of("가입일", "가입일자", "가입년월일", "입부일", "입부시기", "동아리가입일", "joindate")),

    /*
     * 등급·상태는 mbr에서 NOT NULL이고 서버가 정할 근거가 없어 **매핑이 필수**다.
     * 기수(0)·전산 가입일(이관일)처럼 기본값을 둘 수 없다 — 어느 등급으로 넣을지는 명부가 아는 사실이지
