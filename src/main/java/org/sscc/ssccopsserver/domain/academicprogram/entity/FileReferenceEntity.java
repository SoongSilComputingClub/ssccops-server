...
/*
 * file_reference(출석 인증사진) — 회차 하나에 붙는 사진 1장 (#137, 학술관리_데이터모델.md §2).
 * 개인별 태깅이 없는 단순 첨부 증거이며, session_id UNIQUE가 "회차당 1장"을 DB에서 강제한다.
 *
 ...
 * 화면은 fileUrl을 그려 보고 깨지면 다시 올린다(재업로드가 UPSERT라 언제든 다시 부를 수 있다).
 *
 * 감사 컬럼을 두지 않는 것은 session·attendance와 같은 이유다(ERD에 없고, 재업로드가 이력을
...
@Table(
        ...
        uniqueConstraints =
                @UniqueConstraint(
                        name = "uk_file_reference_session",
...
public class FileReferenceEntity {
     ...
     * 이 참조를 다른 회차로 옮기는 일이 아니다(session은 재제출이 회차를 옮길 수 있어 잠그지
     * 않은 것과 갈린다).
     */
     ...
     *
     * 길이를 200이 아니라 500으로 둔 것은 이 값이 사용자가 적어 넣는 주소가 아니라
     * `{공개 도메인}/academic-programs/{id}/sessions/{id}/{uuid}.{ext}`로 조립되는 값이라
     * 공개 도메인이 길어지면 함께 길어지기 때문이다(event.thmb_url_addr는 사람이 붙여 넣는
     * 주소라 200이다).
     */
