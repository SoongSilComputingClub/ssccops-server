...
public enum AuthorityCode {
    ...
    /*
     * 행사 관리 (ssccops#133 · D8). 행사 CRUD·게시 전이·신청 심사·참가자 등록·승격을 하나로
     * 묶는다 — wave2가 권한을 잘게 쪼개지 않기로 했다(필요해지면 자식으로 분리). 시드는
     * OPERATOR의 자식이라 국장 이상 + SUPER가 자동 보유한다. 공개 조회는 익명이라
     * EVENT_READ를 두지 않는다.
     */
    EVENT_MANAGE,

    /*
     * 학술 활동(스터디/프로젝트) 관리 (#130). 기획안·회차·종료 승인, 모집 시작, 회차 승인,
     * 유형 코드테이블 쓰기, 신청자 감독 조회를 아우른다. OPERATOR의 자식이라 국장 이상은
     * 자동으로 포함한다 — 스터디장/팀장은 이 권한 없이 leadrMbrId 소유권 정책으로만 판정한다
     * (역할 기반이 아니라 "이 활동 한정 관계 기반"이라 RBAC 트리에 넣지 않는다).
     */
    ACADEMIC_PROGRAM_MANAGE,

    /*
     * 규정 도우미 코퍼스 관리 (#402 · ADR-0029). 규정 문서의 업로드·재색인·삭제·적용 전환·목록이
     * 전부 이 하나에 걸린다 — 질의는 인증만 요구하고 이 권한은 코퍼스를 바꾸는 쪽에만 붙는다.
     *
     * 기존 권한을 빌려 쓰지 않은 것은 회칙·학칙이 폼도 행사도 회원도 아니기 때문이다
     * (FORM_WRITE를 빌리면 "폼을 만들 수 있는 사람이 규정을 갈아치울 수 있다"가 된다).
     *
     * 시드는 SUPER 직속 + 회장·부회장·총무에 명시 부여다(V11). EXECUTIVE의 자식으로 두지
     * 않은 것은 그러면 좁히는 쪽이 화면 조작이 아니라 마이그레이션이 되기 때문이다 —
     * #101이 SUB_WORK_TYPE_MANAGE를 EXECUTIVE 밑에서 떼어낼 때 치른 값이 그것이다.
     */
    RAG_DOCUMENT_MANAGE,

    /*
     * 공개 사이트 콘텐츠 관리 (ssccops#381 · ADR-0038). 페이지·포스트의 작성·수정·게시·게시
     * 취소·갤러리·개정 이력이 전부 이 하나다 — 행사(EVENT_MANAGE)처럼 쪼갤 자식이 없다.
     * 익명 조회(/public/v1/pages·posts)는 권한이 아니라 게시 상태가 가른다.
     *
     * EVENT_MANAGE를 빌리지 않은 것은 행사와 소개·연혁·아카이브의 주인이 다르기 때문이다
     * (행사는 기획·학술이 만들고 공개 사이트 문안은 홍보국이 쓴다). 시드는 SUPER 직속 +
     * 회장·부회장 명시 부여이며(V16) 홍보국 역할은 시드에 없어 화면에서 켠다.
     */
    CONTENT_MANAGE,
