...
public record ClubJoinPeriod(Integer year, Integer month) {
     ...
     * 모양에도 맞지 않는다.
     *
     * 마지막 `^-+|-+$`는 뜻 그대로다 (#357 · Sonar S5850은 `^(a|b)$`를 의도한 것으로 보지만
     * 여기서는 "앞의 '-' 묶음 **또는** 뒤의 '-' 묶음"을 각각 지우는 것이 의도라 `(^-+)|(-+$)`가
     * 맞다 — '-2020-3-'에서 양끝을 모두 벗겨야 하고 그 사이의 '-'는 남아야 한다). 백트래킹
     * (S8786)도 실질이 없다 — 바로 앞 `-{2,}` → `-` 정리로 '-'가 연속하지 않아 `-+`가 한 글자
     * 이상 물러날 일이 없다.
     */
