...
public enum AssistantErrorCode implements ErrorCode {
    ...
    /*
     * 429 — 적재(#399) 또는 질의(#404)의 한도를 넘었다. 기획안 §11.
     *
     ...
     * 자리다.
     *
     * **네 한도가 코드 하나를 나눠 쓴다** — 적재 일 10회 · 질의 회원 분 5회 · 질의 회원 일 50회 ·
     * 질의 전역 분. 코드를 넷으로 쪼개지 않은 것은 화면이 할 일이 전부 «안내하고 기다린다»로 같고,
     * 갈리는 것은 **얼마나 기다리는가**뿐이라서다. 그 값은 코드가 아니라 문장이므로
     * `GeneralException.detail`이 «잠시 뒤 다시»와 «내일 다시»를 가른다(#150이 정한 방식).
     */
