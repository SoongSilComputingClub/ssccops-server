...
- **운영 도구는 24종이고 세 클래스다** — `OperationTools`(1차 읽기 7 + 전이·체크 2 · #385) · **`WorkTools`**(업무·하위 업무 쓰기 8) · **`MeetingTools`**(회의 쓰기 4 + 승인함·대시보드·유형 읽기 3). 뒤의 둘은 W1(ssccops#365 · Story)이며 어드민 기능 전체로 넓히는 파도의 첫 번째다 — 남은 파도는 학술 → 행사 → 폼 → 회원(+소프트 삭제)이고 회원명·삭제 범위는 [ADR-0037](https://github.com/SoongSilComputingClub/ssccops/blob/develop/docs/decisions/0037-mcp-tool-output-carries-member-names-and-soft-delete-only.md)이 정했다.
  - **수정은 «읽고-합치기»다** — 서버 PATCH가 전체 교체라(F2) 부분 호출이 다른 필드를 지운다. `update_work`·`update_sub_work`는 `WorkPatch`·`SubWorkPatch`(`global/mcp/tool/patch/`)로 **바꿀 것만** 받고 상세를 GET해 빈 자리를 채운 뒤 전체 본문으로 PATCH한다. **도구용 DTO를 쓰지 않는다는 규칙의 유일한 예외**이며(Update record는 제목·담당자가 `@NotNull`이라 «안 바꿈»을 담을 수 없다) 그래서 `McpPatchContractTest`가 필드 이름·타입의 1:1 대응과 nullable 여부를 반사로 검사한다 — 컨트롤러 record가 자라면 테스트가 깨진다. `OperationToolsIntegrationTest`는 «priority만 준 호출이 제목·총평을 남기는지»를 REST 왕복으로 본다. 대가는 GET 한 홉과 **덮어쓰기 경합**(읽은 뒤 남이 고친 값을 되돌릴 수 있다)이며 도구 설명에 적혀 있다. 상태·승인·하위 업무 유형은 patch가 받지 않는다(전이 API가 따로 있고 유형은 소급 금지).
