    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests(
                auth -> {
                     ...
                     * 유일한 층이다.**
                     *
                     * 위의 두 예외(헬스 프로브·Swagger)는 서비스 데이터를 내보내지 않지만
                     * 이쪽은 다르다 — 그럼에도 여는 근거는 목적 자체가 "가입 전 지원자·외부인이
                     * 행사를 보고 신청으로 이어지는 것"이라, 로그인을 요구하면 그 경로가 통째로
                     * 닫히기 때문이다. 폼의 '공개' 경로(/v1/forms/{id}/public)와 갈리는 지점이며
                     * 그쪽은 여전히 인증이 필요하다(응답자는 전원 회원이다).
                     *
                     * **예외를 경로 접두사로 표현한다.** /v1/** 아래에 익명 엔드포인트를 섞으면
                     * 여기가 엔드포인트별 permitAll 목록이 되고, 한 줄이 빠지거나 패턴이 넓게
                     * 잡히는 것으로 인증이 필요한 자원이 열린다. 접두사로 갈라 두면 규칙은 이
                     * 한 줄로 끝나고, 무엇이 익명에게 열려 있는지는 /public/v1 아래 컨트롤러
                     * 목록을 보는 것으로 답이 된다.
                     *
