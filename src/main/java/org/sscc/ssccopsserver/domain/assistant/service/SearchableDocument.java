...
/*
 * 질의가 볼 수 있는 문서 하나 — 인용에 실릴 값만 (#403).
 *
 ...
 * 하는 것과 같은 판단이다(#400).
 *
 * **문서 식별자와 판본 번호가 있던 자리다**(ADR-0034). 앞의 것은 추천 질문(§13.3)이 «그 문서가
 * 지금 코퍼스에 있는가»를 묻는 열쇠였는데, 추천 질문이 고정 셋이 되며 물을 것이 «검색 대상
 * 문서가 하나라도 있는가»로 바뀌어 이 값이 필요 없어졌다.
 *
 * `applyStatus`·`effectiveFrom`은 **화면의 «YYYY-MM-DD 시행 기준» 배지**가 쓰는 값이고
 * (§13.1), `name`은 인용 카드의 문서명이다. 표시명이 색인 시점의 값(청크 메타에 찍힌 것)이
 * 아니라 **지금 값**인 것은 운영진이 화면에서 고친 이름이 답변에 곧바로 반영되는 편이 맞기
 * 때문이다 — 청크 본문의 옛 이름은 재색인이 고친다(#398).
 */
