...
- **규정 도우미 실측 벤치마크**: `./gradlew ragBench` (#465). **로컬 DB에 적재된 코퍼스와 실제 Gemini 임베딩**으로 시험지 한 벌(`support/AssistantBenchmarkSet` · 대화 단위 28턴)을 돌려 검색 재현율을 재고 `build/rag-bench/{label}.tsv`에 남긴다 — `--compare`가 고치기 전후의 **바뀐 판정만** 찍는다. `geminiCheck`·`r2Check`와 같은 자리라 `./gradlew test`에 섞지 않는다(실제 자격증명과 적재된 DB가 있어야 답이 나온다). 기본은 채팅 모델을 스텁으로 갈아 **쿼터를 쓰지 않으며**(시험지 한 바퀴가 무료 티어 하루치다) `--answer`로만 실제 모델을 부른다. 왜 골든셋으로 부족한지와 군별 실측값은 `domain/assistant/AGENTS.md`에 있다.
...
검증한다** — 빈 DB에 V1부터 전부 적용하고 엔티티 47종에 대해 `validate`가 통과하는지 본다.
...
- **머지는 곧 dev 배포다**(#202 · #569 — `develop` 푸시가 CI를 통과하면 `deploy-dev.yml`이
  이미지를 올리고 Coolify가 받아 띄운다). 마이그레이션이 깨지면 dev가 즉시 죽으므로
  `FlywayMigrationValidateTest`를 통과시키고 머지한다. **그 테스트는 CI에 있으니 실패하면
  이미지가 만들어지지도 않는다** — 옛 경로(Coolify가 푸시 웹훅을 받아 바로 빌드)에서는
  깨진 커밋이 그대로 dev에 올라갔다.
...
| `notification` | 알림(`noti`)·푸시 구독(`push_sbscrp`) — 하위 업무 전이(운영의 `SubWorkTransitionedEvent`) · 폼 응답 검토(`FormResponseReviewedEvent`) · 행사 참가 상태(`EventParticipantStatusChangedEvent`)를 AFTER_COMMIT + `@Async`로 듣고, 09:00 마감 스케줄러와 테스트 알림(`POST /v1/notifications/test` · 1분 3회)이 알림 행을 만들어 표준 Web Push(VAPID · RFC 8291/8292 직접 구현)로 보낸다 · 수신 앱은 **기준표 `noti_type_rcpn`**이 정하고 행이 없으면 보낸 앱이다(ADR-0047) · `/v1/notifications`(`app` 필터) · `/v1/push/*` · 인증만, **`/v1/notifications/types`만 `SUPER`** | [domain/notification/AGENTS.md](src/main/java/org/sscc/ssccopsserver/domain/notification/AGENTS.md) |
...
- `global/security` — 인증/인가. 구글 OAuth2 로그인과 자체 JWT 발급 스택은 제거됐고, 지금은 Supabase Auth가 발급한 JWT를 `spring-boot-starter-oauth2-resource-server`로 검증한다. JWKS URI(`{SUPABASE_URL}/auth/v1/.well-known/jwks.json`)는 프로필별로 다른 Supabase 프로젝트를 가리키도록 `application-{profile}.yaml`에 분리돼 있다.
  ...
  - `SecurityConfig`가 필터체인을 구성: CSRF/formLogin/httpBasic/logout 비활성화, `SessionCreationPolicy.STATELESS`, 비prod Swagger 경로와 **`/actuator/health/liveness`·`/actuator/health/readiness`**·`/actuator/info`, 그리고 **`/public/v1/**`**만 permitAll(배포 헬스 프로브는 토큰을 붙일 수 없다 — `prometheus`·`metrics`는 계속 인증을 요구하고 **`loggers`는 노출 목록에서 빠졌다**: 그것은 조회가 아니라 `POST`로 런타임 레벨을 바꾸는 **쓰기**였고 `@RequireAuthority`가 애노테이션 AOP라 닿지 않아 유효한 토큰 하나가 전부였다 — 2026-09-24 dev 실측으로 확인하고 #555에서 걷었다. 익명 표면을 `/actuator/health` 전체에서 프로브 둘로 좁혔고 상세는 `when-authorized`다. ⚠️ **`readiness`가 DB를 보는 것은 `application.yaml`의 `management.endpoint.health.group.readiness.include`가 하는 일이다** — 그 줄이 사라지면 ADR-0022의 실질이 뒤집혀 autoheal이 DB 장애에 반응하지 않는다), 그리고 **`/.well-known/oauth-protected-resource`**(#384 · MCP 클라이언트가 401 뒤에 읽는 RFC 9728 메타데이터 — 서비스 데이터가 아니라 설정에서 온 상수라 헬스 프로브·Swagger 부류다, 아래 «MCP» 절), 나머지는 인증 필요. **`/public/v1/**`는 업무 API 중 익명 접근이 허용되는 유일한 층이다**(#156 · 공개 행사 조회, ssccops#143 D1 · 행사 이미지 리다이렉트 `PublicEventImageController`, #208 — 본문 이미지는 상세와 같은 화면에서 열리므로 그쪽만 토큰을 요구하면 화면이 성립하지 않고, 카카오톡·에브리타임 OG 크롤러도 이 주소를 열어 302를 따라간다) — 예외를 엔드포인트 목록이 아니라 경로 접두사로 표현한 것은, `/v1/**` 아래에 익명 경로를 섞으면 여기가 경로별 permitAll 표가 되고 한 줄이 빠지거나 패턴이 넓게 잡히는 것으로 인증이 필요한 자원이 열리기 때문이다. 그러므로 **이 접두사 아래에 핸들러를 더하는 것은 permitAll을 더하는 것과 같다**. **콘텐츠 셋이 여기 더해졌다**(ssccops#381 · [ADR-0038](https://github.com/SoongSilComputingClub/ssccops/blob/develop/docs/decisions/0038-anonymous-content-layer-pages-posts-open-forms.md)) — 게시된 페이지 `GET /public/v1/pages/{slug}` · 게시된 포스트 목록·상세·갤러리 이미지 `GET /public/v1/posts…`(`PublicContentController`) · 접수 중인 폼 목록 `GET /public/v1/forms/open`(`PublicFormMetaController`), 그리고 **신입회원 모집 지정 폼 메타 `GET /public/v1/forms/system/RECRUIT/meta`**(#520 · ADR-0044 · 같은 컨트롤러 — RECRUIT만, 다른 코드·지정 전·DRAFT는 404 · `domain/form/AGENTS.md`). 응답은 «게시 상태인 것의 공개용 필드»로 가둔 별도 record(`Public*`)이고 `PublicContentDtoContractTest`가 그 컴포넌트를 금지 목록과 대조하며, `Cache-Control: public, s-maxage=300, stale-while-revalidate=600`(`global/apipayload/PublicCacheControl`)이 실린다. 집계값·학술 목록은 «게시»라는 상태가 없어 이 층에 오지 않는다 — 넓히는 것은 새 ADR이다. 폼의 '공개' 경로(`/v1/forms/{id}/public`)와 갈리는 지점이며 그쪽은 여전히 인증이 필요하다(응답자는 전원 회원이다). **경로별 권한 규칙을 여기 두지 않는다** — `RoleHierarchy`(ADMIN ⊃ USER ⊃ PREUSER)와 `/admin/**`은 #9에서 제거했다(실제 어휘와 대응되지 않는 잔재였고, 남기면 권한 어휘가 두 벌이 된다).
...
- **도구는 67종이고 여덟 클래스다** — `OperationTools`(1차 읽기 7 + 전이·체크 2 · #385) · **`WorkTools`**(업무·하위 업무 쓰기 8) · **`MeetingTools`**(회의 쓰기 4 + 승인함·대시보드·유형 읽기 3) · **`EventTools`**(W2 · #494 + W5 · #589 — `list_events`·`get_event`·`create_event`·`update_event`(`EventPatch` 읽고-합치기 · 서버가 PUT이라 `McpRestClient.put`)·`change_event_status`(PUBLISH·RETRACT·ARCHIVE·REPUBLISH)·**`delete_event`·`restore_event`** — 복제·이미지·공유 도구는 없다) · **`FormTools`**(W2 · #494 + W4 · #587 + W5 · #589 — `list_forms`(`receiptStatuses` 다중)·`get_form`(문항 포함)·`change_form_status`(OPEN·CLOSE)·`list_form_labels`·`assign_form_labels`(**전체 교체** — 하나를 더하려면 지금 걸린 것을 함께 보낸다)·`list_form_responses`·`review_form_response`(ACCEPTED·CHANGES_REQUESTED·REJECTED · 수정요청·반려는 의견 필수 · 승인·반려는 종결이라 되돌릴 수 없다)·**`delete_form`·`restore_form`** — **문항을 만들거나 고치는 도구와 응답 상세 도구는 없다**, 아래) · **`AcademicTools`**(W3 · #567 — 활동·회차·기획안 검토·모집 선발·출석 정정 12 · 만들기 도구는 없다: 활동은 기획안 승인 이관으로만 생긴다) · **`MemberTools`**(W5 · #589 · ADR-0037 — `list_members`·`get_member`·`change_member_grade`·`change_member_status`·`list_member_histories`·`list_roles`·`list_member_roles`·`assign_member_role`·`update_member_role_assignment` 9 · 아래) · **`ContentTools`**(콘텐츠 6 — `create_page`·`update_page`·`create_post`·`update_post`·`publish_content`(kind=page|post + publish 불리언, 출력은 kind에 따라 `ContentPageResponse`/`ContentPostResponse`)·`request_content_image_upload`(presigned PUT + `fileId` — 바이트는 스크립트가 올린다) · ssccops#381 · ADR-0038 · 규칙은 `domain/content/AGENTS.md`). **ssccops#365의 파도 다섯이 전부 끝났다** — W1 운영 → W2 행사·폼 → W3 학술 → W4 폼·응답 → W5 회원·삭제. 남은 범위 밖 항목은 폼 문항 편집 · CSV 이관 · 권한 트리 편집 · 하드 삭제 · 이미지 업로드 · CSV 내보내기 · 규정 도우미 질의다.
  - **수정은 «읽고-합치기»다** — 서버 PATCH가 전체 교체라(F2) 부분 호출이 다른 필드를 지운다. `update_work`·`update_sub_work`·`update_page`·`update_post`·`update_event`는 `WorkPatch`·`SubWorkPatch`·`ContentPagePatch`·`ContentPostPatch`·`EventPatch`(`global/mcp/tool/patch/`)로 **바꿀 것만** 받고 상세를 GET해 빈 자리를 채운 뒤 전체 본문으로 PATCH한다. **도구용 DTO를 쓰지 않는다는 규칙의 유일한 예외**이며(Update record는 제목·담당자가 `@NotNull`이라 «안 바꿈»을 담을 수 없다) 그래서 `McpPatchContractTest`가 필드 이름·타입의 1:1 대응과 nullable 여부를 반사로 검사한다 — 컨트롤러 record가 자라면 테스트가 깨진다. `OperationToolsIntegrationTest`는 «priority만 준 호출이 제목·총평을 남기는지»를 REST 왕복으로 본다. 대가는 GET 한 홉과 **덮어쓰기 경합**(읽은 뒤 남이 고친 값을 되돌릴 수 있다)이며 도구 설명에 적혀 있다. 상태·승인·하위 업무 유형은 patch가 받지 않는다(전이 API가 따로 있고 유형은 소급 금지).
  - **응답 상세를 내주는 도구는 없다 — 결정이다**(#587). `FormResponseDetailResponse.rspnsCn`은 `ResponseContent` = **`Map<String, Object> answers`**이고 **키가 문항 id**라 `ToolOutputRedactor`의 이름 기반 마스킹이 **그 맵 안에 닿지 않는다.** 문항은 운영진이 자유롭게 만들므로 연락처·주소·자기소개를 묻는 문항이 있으면 그 값이 그대로 나가고, **서버는 그 답에 무엇이 들었는지 모른다.** ADR-0037이 못 박은 «이름은 싣고 연락처·이메일·학번은 지운다»는 서버가 모양을 아는 필드에 대한 규칙이라 답 원문을 덮지 못한다. 그래서 여는 것은 **목록과 심사**이고(목록은 대표 문항의 답 `responseTitle`까지 준다 — 선언은 `SystemFormContract`), 답을 읽어야 결론이 나는 심사는 어드민 화면에서 한다. `EventFormToolsIntegrationTest.noFormResponseDetailTool`이 도구 목록에 `get_form_response`가 없음을 지킨다 — 이름만 보고 «빠뜨렸네»로 더하는 것을 막기 위해서다. 열려면 rspnsCn을 통째로 비우는 규칙을 redactor에 더하거나(그러면 상세가 목록과 거의 같아진다) 새 ADR이 선행이다.
  - **삭제 도구는 되살리는 API가 있는 둘뿐이다 — `delete_form`·`delete_event`**(#589 · [ADR-0053](https://github.com/SoongSilComputingClub/ssccops/blob/develop/docs/decisions/0053-mcp-delete-tools-only-where-restore-api-exists.md)), 그리고 **되살리기를 같은 파도에서 함께 냈다**(`restore_form`·`restore_event`). ADR-0037은 «소프트 삭제만 연다»의 근거로 «되살릴 수 있어 «확인»을 습관적으로 누른 대가가 복구 가능하다»를 들었는데 **다섯 중 셋에는 사실이 아니었다** — `OperationEntity`에는 `softDelete`만 있고 **`restore`가 아예 없어** 업무·하위 업무·회의는 지우면 DB를 직접 고치지 않는 한 돌아오지 않는다(업무 삭제는 `deleteWork`가 하위 업무까지 연쇄로 지운다). 어드민 휴지통 화면도 `deleted-form-list`·`deleted-event-list` 둘뿐이다. 그래서 근거가 성립하는 범위로 도구를 맞췄고 **하드 삭제는 어느 파도에서도 열지 않는다**(ADR-0037). 넷 다 `destructiveHint = true`. 되살리기는 서버가 `ApiResponse<Void>`를 주므로 **되살린 뒤 상세를 한 번 더 읽어** 돌려준다 — 그 조회가 실패하면 도구는 오류로 보이지만 대상은 이미 돌아와 있다. `advertisesEveryOperationTool`의 단정은 «`delete_`가 하나도 없다»에서 **«`delete_work`·`delete_sub_work`·`delete_meeting`·`delete_member`가 없다»**로 좁혔다 — 되살리기 API가 생기면 그때 이 목록에서 빼는 것이 «다시 계산했다»는 표시가 된다.
  - **회원 도구는 9종이고 없는 것에 이유가 있다**(#589 · ADR-0037). 목록·상세·등급·상태·이력과 역할 목록·부여·종료다. **일괄 등급·상태 변경은 없다** — 한 호출이 여러 명을 바꾸고 부분 성공이 정상 응답이라, 모델이 «2학년 전부 정회원으로» 같은 **대상 집합을 스스로 만들면** 틀린 집합 하나가 되돌리기 어려운 변경 여러 건이 된다(화면은 사람이 체크박스로 고른 것이 집합이다). **회원 정보 수정(`PATCH /v1/members/{id}`)도 없다** — 이름·학번·연락처를 고치는 자리라 **출력에서 지운 값을 입력으로 받는 도구**가 되어 ADR-0037과 어긋난다. **역할·권한 트리 편집도 없다**(ssccops#365 범위 밖) — `list_roles`만 있는 것은 부여에 필요한 `roleId`를 얻기 위한 읽기다. 그리고 **목록 상한을 도구가 다시 고정하지 않는다**: ADR-0037이 «명부 전량이 한 호출로 나가지 않게»를 따라오는 일로 적었지만 `MemberSearchCondition`이 `@Max(100)`·기본 20으로 **서버에서** 이미 막으므로, 도구에도 적으면 상한이 두 곳에 생겨 갈린다. `MemberToolsIntegrationTest`가 실제 왕복에서 이름은 나오고 연락처·학번은 나오지 않는지 본다.
  - **회원명은 출력에 남는다**(ADR-0037) — `ToolOutputRedactor`가 지우는 것은 전화번호·이메일·학번의 **모든 표기**(`phoneNumber`·`email`·`studentNumber` + `telno`·`eml`·`stdntNo`)이며 이름을 지우면 회원 도구가 성립하지 않는다. 그 목록에 이름 필드를 더하는 것은 그 ADR을 뒤집는 것이다.
  - 1차 도구 이름과 REST 짝:
  ...
  - **개인정보는 `ToolOutputRedactor`가 JSON 트리에서 이름으로 걷어낸다** — 전화번호 · 이메일 · 학번(깊이 무관, `MemberSummaryResponse`가 담당자·등록자·발표자 자리마다 중첩되어 있어서다). 이름(`name`)은 남긴다 — 담당자가 누구인지 없이는 운영 도구가 성립하지 않는다. 도구마다 출력용 뷰 record를 만들지 않은 것은 응답 record에 필드가 늘 때 한쪽만 늘어 갈리고 새 도구가 뷰를 빠뜨리면 조용히 새기 때문이다. 걷힌 필드는 출력에 `"phoneNumber":null`처럼 값 없는 키로 남는다(mcp-annotations 직렬화는 null을 빼지 않는다). 키를 더하면 이 표도 고친다.
    - ⚠️ **이름이 두 벌이라 목록도 여섯이다** (#567) — 회원 도메인 DTO 는 `phoneNumber`·`email`·`studentNumber` 를, 나머지는 데이터사전 약어 `telno`·`eml`·`stdntNo` 를 쓴다. 처음 셋만 걸러 두었던 동안 「빠뜨릴 도구가 없다」가 **약어 쪽 record 에는 성립하지 않았다**(`ResponseMemberSummary`·`ResponseMemberDetail`·`RoleDetailResponse`). 도구가 그것들을 돌려주지 않아 새지는 않았지만 모집 지원 목록·응답 심사·회원 도구가 열리는 순간 새는 자리였다. 지금은 **`ToolOutputRedactorCoverageTest` 가 `domain/…/dto` 전수를 훑어** 새 이름이 생기면 깨진다 — 규칙을 문장이 아니라 대조로 옮긴 것은 `file_rfrnc` CHECK(#443)과 같은 처방이다.
...
- ⚠️ **사고 수준을 명시한다 — `GEMINI_CHAT_THINKING_LEVEL`(기본 `MEDIUM`)** (#453 · #463).
  ...
  **그리고 #463이 그 기각을 되돌려 기본값이 `MEDIUM`이 됐다 — 두 실측은 어긋나지 않는다.
  시험지가 달랐다.** 위 넷은 전부 내용 질문이고 **인용 요청 계열이 없었다.** 「그대로 인용해
  달라」를 세게 적으면 `MINIMAL`이 **근거를 쥐고도 거절한다**(2026-09-17 · 일괄 경로 · 초는
  생성 구간 = `소요 − 검색`):

  | 인용 요청 | MINIMAL | MEDIUM |
  |---|---|---|
  | 회칙 제3조의 내용을 그대로 인용해줘 | 4/4 답 · 0.87~1.20s | 1/1 답 · 2.80s |
  | 제7조 원문을 그대로 보여줘 | 1/1 답 · 1.60s | 1/1 답 · 3.19s |
  | 회칙 제21조의 내용을 그대로 인용해줘 | 1/1 답 · 0.98s | — |
  | **제21조 전문을 토씨 하나 바꾸지 말고 옮겨 적어줘** | **0/4 — 전부 거절** | **4/4 답** |

  마지막 줄에서도 발췌에는 그 조가 **#457의 핀으로 들어가 있었다**(`발췌=6` · 거절 사유는
  «모델이 근거 없음을 표식으로 알렸다») — 검색 실패가 아니라 **프롬프트 준수 실패**이고,
  규칙 3(`[근거없음]`)과 규칙 5(인용 요청은 거절하지 말고 간추려 답하라)가 충돌하는 자리다
  (`AssistantPrompt` · #457이 그 둘 사이에 한 줄을 이미 넣어 둔 자리이기도 하다).
  **갈리는 것은 조가 아니라 요구의 세기**라 같은 제21조도 「그대로 인용해줘」 정도면
  `MINIMAL`이 답한다. 값은 생성 구간 **2.5~3.3배**이고 **`LOW`는 아직 재지 않았다** —
  내용 질문만 놓고 보면 위 표가 여전히 사실이므로, 되돌리려는 사람은 두 시험지를 함께 볼 것.
...
- 브랜치: 이슈 생성 시 `issue-branch-creator.yml`이 제목 앞 태그(`[FEAT]`/`[FIX]`/`[REFACTOR]`/`[CHORE]`)를 읽어 **`{type}/#{이슈번호}`** 형식으로 자동 생성(#502 · ssccops#419). **슬러그는 없다** — 예전의 `-{영문 슬러그}`는 LibreTranslate 컨테이너가 한글 제목을 번역해 만들었는데 그 단계가 워크플로의 병목이었고(모델 로딩 대기 + 가드 넷, #208 · #211), 사람·스크립트가 로컬에서 먼저 딴 이름과 봇의 이름이 달라 원격 브랜치가 둘이 되는 일이 되풀이됐다. 이름이 번호로 결정적이라 이제 누가 먼저 만들든 같은 브랜치다. 옛 `{type}/#N-슬러그` 브랜치는 pr-guard가 계속 받는다. 직접 만들어야 한다면 같은 형식을 따르며, **남의 작업 브랜치가 아니라 `develop`에서 딴다** — #235가 작업 중이던 다른 브랜치 위에서 갈라져 나오는 바람에 문서 한 줄짜리 PR이 남의 61개 파일을 함께 머지했다.
