 ...
 *     그 배수가 된다. ⚠️ **«단일 인스턴스 배포라 성립한다»는 문장은 2026-09-24에 걷었다** —
 *     Coolify는 새 컨테이너를 healthy로 만든 뒤 옛 것을 내리므로 **배포 중에는 둘**이고, 그
 *     몇 분 동안 실질 상한이 5회가 아니라 10회다(#556 · ssccops#501). 배포는 develop 푸시마다
 *     일어난다.
 *     <p>그래도 인메모리로 두는 것은 **겹침이 몇 분이고 그 창에서 두 배가 되는 것을 감수**하는
 *     쪽이, 이 하나를 위해 공유 저장소를 운영하는 것보다 싸기 때문이다(ADR-0028의 같은 저울).
 *     상시 여러 인스턴스가 되면 그때 Redis로 옮긴다.
