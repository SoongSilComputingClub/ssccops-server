...
- 서비스: 포트 `RagChunkStore`와 그 구현 `PgVectorRagChunkStore` · 기능 플래그 `AssistantFeature` ·
  ...
  `QueryEmbeddingCache`(#406) · 흘러나오는 답을 받는 자리 `AssistantAnswerSink`와 받는 쪽이
  사라졌음을 알리는 `AssistantStreamClosedException`(#447).
- 컨트롤러: `RagDocumentController`(코퍼스 6핸들러 · 클래스 레벨
  `@RequireAuthority(RAG_DOCUMENT_MANAGE)`)와 **`AssistantController`(질의 둘·추천 질문·대화
  초기화 · 인증만)**.
...
- 확장자 표는 `code/RagDocumentFormat` 한 곳이다 — **파서(`Extraction`)와 «회칙 후보인가»를 가른다.**
  **유형은 여기서 갈리지 않는다**(#445) — 파싱 결과가 정하며 그 자리는 `RagDocumentServiceImpl.resolveType`이다.
...
- **유형은 파싱 결과가 정하고, 그것이 인용의 모양을 정한다**(#445). `.md`는 회칙 파서를
  **시도**해 통과하면 `STRUCTURED`(조항 인용)이고 계약을 어기면 `GENERIC`(문서명 인용)으로
  떨어진다 — 나머지 확장자는 곧바로 `GENERIC`이다. **거절이 아니라 이동이며**, 잃는 것은 그
  문서의 조 단위 인용뿐이고 내용은 그대로 검색·인용된다. 그전에는 확장자가 유형을 단정해
  «회칙이 아닌 `.md`는 코퍼스에 들어갈 방법이 없는» 상태였다.
- **떨어진 것은 조용하지 않다** — `RagDocumentResponse.docType`이 목록에 실려 화면이 그린다.
  회칙 `.md`에 오타가 나서 평문으로 떨어져도 운영진이 본다. **되묻는 버튼이나 요청 플래그를
  두지 않은 이유가 이것이다** — 그것이 있으면 «턱턱 넣는다»를 한 번 더 막는 값만 남는다.
- **요청이 유형을 신고하지 않는다**(#210과 같은 판단). 판정 근거가 「확장자」에서 「확장자 +
  파싱 결과」로 넓어졌을 뿐 **둘 다 서버가 파일에서 직접 읽는 값**이라, 화면이 고른 유형과 실제
  파일이 다른 상태는 여전히 만들어지지 않는다.
- **유형을 정하는 자리가 하나여야 한다.** 색인 워커가 `rag_doc.doc_type_cd`로 청커를 고르므로
  (`RagIndexingWorker.chunk`) 업로드가 정한 값을 행에 적어 둔다 — 두 곳이 따로 판단하면 «업로드는
  회칙으로 읽었는데 워커는 평문으로 자르는» 상태가 성립한다.
- 평문에서 «제○조»를 정규식으로 긁어 조항 인용을 흉내 내지 말 것 —
  **맞을 때도 틀릴 때도 있는 인용은 없는 인용보다 나쁘다.**
...
| **업로드도 같은 규칙이다** | `RagDocumentServiceImpl.upload`가 «검증·한도 → (Tika 파싱) → 적기»로 갈려 있고 역시 `TransactionTemplate`이다 (#563 · ssccops#512). 그전에는 `@Transactional`이라 **파싱과 10MB R2 PUT이 끝날 때까지 커넥션을 쥐었다** — 워커에서 피한 것과 같은 고장이 업로드 경로에만 남아 있었다. 다만 **행·PUT·`file_rfrnc`는 한 트랜잭션에 함께 둔다**: 그 셋은 «실패하면 아무것도 남지 않는다»는 계약을 함께 지고 있어 갈라 놓으면 원본 없는 문서 행이 생긴다. 한도 판정이 두 번인 것은 파싱이 그 사이에 끼어 창이 벌어지기 때문이다(앞은 비싼 파싱을 막는 미리 보기, 뒤가 진짜 판정) |
...
- ⚠️ **배포 중에는 인스턴스가 둘이다** (2026-09-24 실측 · #556 · ssccops#501). Coolify는 새
  컨테이너를 띄워 헬스체크를 통과시킨 뒤 옛 것을 내리므로 **겹침은 사고가 아니라 배포 절차 그
  자체**이고, 배포는 `develop` 푸시마다 일어난다. 그전에 이 자리에 «다중 인스턴스에서는 성립하지
  않는다»고만 적혀 있던 것은 «상시로 둘은 아니다»를 «둘이 될 일이 없다»로 읽은 것이었다.
  - `INDEXING`은 «누가 하고 있다»를 말할 뿐 «누가»를 말하지 않아 새로 뜬 인스턴스의 부팅 복구가
    **남의 진행 중 작업을 되돌렸다.** 지금은 **나이 조건**이 막는다 — `indx_bgng_dt`가
    `ssccops.assistant.indexing.stuck-after`(기본 `PT10M`) 이전인 행만 되돌린다. 폴링이 `PT10S`이고
    색인 한 건이 «수십 초»라 정상 진행 중인 작업은 그 창을 넘지 않는다.
  - **«누가 집었는가» 컬럼은 여전히 없고, 지금은 필요하지도 않다.** 그것은 **상시** 다중 인스턴스가
    될 때의 설계이며 배포 겹침에는 나이 조건으로 충분하다(ssccops#324는 그 전제에서 열려 있다).
...
- **하드 삭제는 행 · 청크 · R2 오브젝트 셋이다.** `file_rfrnc` 행과 오브젝트는 파일 도메인의
  ...
  - ⚠️ **색인 중 삭제는 고아 청크를 남길 수 있다.** 워커는 잠금 밖에서 적재하므로, 임베딩이
    ...
    청크는 검색되지 않지만 지울 주체도 없다 — 드물어 감수하는 값이며, 막으려면 워커에 «내가
    집었다»를 적는 컬럼이 필요하다(위 배포 겹침 항목과 같은 자리). **«인스턴스가 하나라서 드물다»는
    근거는 2026-09-24에 걷었다** — 배포 중에는 둘이고, 그래서 나이 조건이 들어갔다(#556).
...
- **계약 위반은 `RAG_DOCUMENT_PARSE_FAILED` + «몇째 줄이 왜»이고, 업로드는 그것을 400으로
  내보내지 않는다**(#445). 「이 파일은 회칙이 아니다」라는 판정이지 「못 읽는다」가 아니므로
  서비스가 그 예외를 받아 평문 경로로 넘긴다 — **파서는 그대로 던지고, 무엇을 할지는 부르는 쪽이
  정한다.** 사유는 `GeneralException.detail`이고 **사유마다 코드를 만들지 않는다**(#150).
  걸리는 것: 조 없음 · 장 없이 시작하는 조 · 조번호 중복 · 조 밖의 항 · 계약에 없는 제목 모양 ·
  모르는 마커 · 마커 둘 이상.
...
받아 온 파일을 **평문 + 페이지 경계**로 뽑는 것이 `GenericTextExtractor`, 그것을 600자·overlap
100자로 자르는 것이 `DocumentChunker`다. 의존성을 어떻게 골랐는지는 루트 `AGENTS.md`의 «Tika»
절에 있다.

**#445에서 확장자가 늘었지만 파서는 셋뿐이다.** 새로 받는 `.pptx`·`.xlsx`는 `.docx`와 같은
`OOXMLParser`이고, 옛 `.doc`·`.ppt`·`.xls`만 `OfficeParser` 하나를 더한다 — 둘 다 이미 깔린
`tika-parser-microsoft-module`이 싣는 것이라 **새 의존성이 없다.** `.md`·`.txt`는 **파서를 아예
쓰지 않는다**: 바이트가 곧 본문이라 Tika를 태우면 파서 수를 줄인 #398의 판단과 거꾸로 간다.
UTF-8로 읽고 BOM을 떼는 것은 `RegulationParser`의 계약과 같으며(#400), 같은 `.md`를 두 파서가
다르게 읽으면 평문 경로에서만 첫 글자가 달라지는 자리가 생긴다. 다른 인코딩은 추측하지 않는다.

**`.hwp`는 받지 않는다** — Tika에 한글 파서가 없어 모듈 추가로 되지 않고 외부 라이브러리 판단이
필요해 #445에서 뗐다. 모르는 확장자와 같은 400 `RAG_DOCUMENT_UNSUPPORTED_TYPE`이다.

- **확장자 표는 `RagDocumentFormat` 한 곳이다.** 같은 표에 여러 질문이 걸린다 — 업로드(#399)는
  «이 파일을 어떻게 여는가»와 «회칙 파서를 시도할 만한가», 추출기는 «어느 파서인가», R2는
  «어느 Content-Type으로 저장하는가». 유형 enum에 얹으면 답할 수 없는 질문이 생겨 추출기가
  확장자를 한 번 더 읽고, **그 순간 표가 두 벌이 된다.** 모르는 확장자는 400이다.
  **오류 문구가 그 표를 그대로 읽으므로**(`supported()`) 확장자를 더할 때 문구를 따로 고치지
  않으며, 반대로 문구만 고치고 표를 빠뜨리는 일도 생기지 않는다 — `RagDocumentFormatTest`가 본다.
...
## 질의 — 방어선 셋 · 검색 필터 둘 · 인용 해석 (#403 · #447 · 기획안 §6 · §10 · §13.3)
...
| 3차 | **모델이 쓴 출처가 우리가 넣어 준 발췌의 번호인지 본다** · **`[근거없음]` 표식을 읽는다** (#455) | `CitationVerifier` |
...
거절은 **넷 다 같은 모양**이다 — 200 · 정해진 문구(`AssistantPrompt.NO_EVIDENCE`) ·
`answered: false` · **빈 배열(null 아님)** `citations` · 판본 값 둘 `null`. 오류가 아닌 것은
화면이 그 문구를 말풍선으로 그려야 하기 때문이고, 넷을 가르는 값은 **로그에만** 남는다
(«모델이 근거 없는 답을 했습니다»를 사용자에게 말할 이유가 없다).

#### ⚠️ «인용이 하나라도 있으면 답한 것»이 아니다 (#455)

넷째가 여기서 나왔다. 프롬프트 규칙 3이 처음에 **«"근거를 찾지 못했습니다"라고 말합니다»**였고,
모델은 그것을 지키면서 **규칙 2도 함께 지켰다** — 「근거가 없다」도 주장이라 출처를 붙였다. 나간
답이 이것이다:

> 규정 문서에서 회칙 제3조의 내용을 그대로 인용할 만한 근거를 찾지 못했습니다. … `[1][2][3][4][5]`

인용 다섯 개가 전부 범위 안이므로 3차 방어선은 통과였고, 「답했는가」의 판정이
`citations.isEmpty()` 하나였으므로 **답변으로 셌다** — 화면에는 판본 배지와 인용 카드 다섯 장이
달린 「찾지 못했습니다」가 그려졌다. **모델이 규칙을 어긴 것이 아니라 두 규칙이 충돌한 자리였다.**

그래서 거절을 **구조로** 받는다. 규칙 3이 시키는 것은 이제 말이 아니라 **답의 첫 줄에 `[근거없음]`
하나**이고, `CitationVerifier`가 그 표식을 보면 본문에서 지우고 `Verified.refused`를 세우며 **그
뒤 조각은 내보내지 않는다.** 그때 `citations`는 비운다 — «근거가 다섯 개인 거절»이라는 상태를
남겨 두면 다음 사람이 그것을 다시 답변으로 센다.

- **표식이 인용을 이긴다.** 모델이 표식을 쓰고도 출처를 마저 달면(규칙 2를 끝까지 지킨 모양이다)
  인용이 몇 개든 거절이다.
- **사용자가 읽는 것은 모델의 거절 문장이 아니라 서버의 `NO_EVIDENCE`다.** 그래야 «질문을 조금
  더 구체적으로 적어 주세요»가 함께 나간다 — 모델의 문장에는 그것이 없었다.
- **프롬프트와 코드가 짝이다**(`AssistantPrompt` 규칙 2·3 ↔ `CitationVerifier`). 한쪽만 고치면
  아무 일도 일어나지 않으므로, 규칙 2의 «대괄호 안에는 숫자 하나»에 이 표식을 **유일한 예외**로
  적어 두었다.
- **로그는 둘을 가른다** — «모델이 근거 없음을 표식으로 알렸다»(규칙을 지킨 것이라 셀 일이 없다)와
  «모델의 답에서 검증을 통과한 인용이 없다»(늘면 프롬프트·임계값을 다시 본다).
...
### 조 지목 핀 — 「제3조」도 「그 다음 조」도 같은 길로 실린다 (#457 · #465)

**밀집 임베딩은 「제3조」를 식별자로 잡지 못한다.** 조 번호가 아니라 「회칙·의결·개정」 같은
주변 어휘로 매칭되기 때문이고, 조 단위 청크가 짧은 것(평균 180자 · 최소 38자)이 겹쳐 661자짜리
평문 청크와 같은 공간에서 경쟁하면 밀린다. 2026-09-17 실측에서 「회칙 제3조는 무엇을 정하고
있어?」의 1위가 **부칙 제4조**였고 정답은 8위라 `top-k` 밖이었다.

그래서 질문이 조를 지목하면 **메타데이터로 그 조를 직접 집어 발췌의 맨 앞에 놓는다**
(`AssistantServiceImpl.pinnedArticle` · `articleNumber` + `supplementary` 필터 · 가지 번호는
질문이 적었을 때만 조건에 넣는다). 이 발췌는 **유사도 판정을 지나지 않는다** — 지목은 그 자체로
근거이지 점수로 다툴 것이 아니다. 예산도 `top-k`와 별도다(조 하나에 청크 최대 3개).

**무엇을 집을지는 `ArticleReference`가 정하고, 거기에 길이 둘이다.**

| 질문이 | 무엇으로 | 예 |
|---|---|---|
| 조를 적었다 | 그대로 — **둘까지** | 「제18조와 제19조는 어떻게 다른가」 → 제18조 · 제19조 |
| 앞 턴의 조를 가리켰다 | **기준 조에서 옮겨** — 하나 | 「그 다음 조는?」 → 기준 +1 |

- **명시가 언제나 이긴다.** 「제7조의 다음 조」는 제7조다 — 두 신호를 합쳐 제8조로 읽으면
  「제7조는 제8조와 어떻게 다른가」가 조용히 옮겨지고, 그 실패는 «엉뚱한 조가 근거로 실렸다»로만
  보인다.
- **둘까지인 이유는 예산이 아니라 프롬프트 길이다.** 지목 발췌는 `top-k`를 밀어내지 않으므로
  맥락을 잃지 않는다 — 대신 프롬프트가 길어지고 그 값이 곧 지연이다(#448이 `top-k`를 8에서 5로
  좁힌 축). 실제로 둘을 나란히 놓고 묻는 질문이 여기까지이고, 셋을 적는 질문(「제23조부터
  제25조까지」)은 가운데가 밀집 검색으로 따라온다(실측).
- **기준 조는 «앞 턴이 실제로 인용한 조»다** — 앞 질문의 글자가 아니다. 「정회원 승격 조건은?」
  에는 조 번호가 없고 그 근거(제7조)는 **답변만 안다**. 저장은
  `AssistantConversations`(대화별 캐시 · 이력과 같은 손잡이)이고, 조항이 아닌 근거로 답한 턴은
  **기준을 지우지 않는다**(평문 문서 한 번이 사슬을 끊지 않게).
- ⚠️ **가지 번호는 옮길 때 버린다.** 제27조의2의 「다음 조」는 제28조라 맞지만, 제28조의
  「이전 조」는 제27조가 되어 제27조의2를 건너뛴다. 가지까지 세려면 코퍼스에 무엇이 있는지를
  알아야 하는데 `ArticleReference`는 질문만 본다 — 그 한 칸은 밀집 검색이 메운다.
- ⚠️ **상대 표현을 넓게 잡지 말 것.** 핀은 임계값을 지나지 않으므로 여기서 잘못 잡은 조는
  **무엇도 걸러 주지 않는다.** 「그 조건은?」·「그 조직은?」·「다음과 같이」·「다음 각 호」가
  걸리지 않는 것은 패턴이 `조` 뒤에 조사·부호·공백·끝만 받기 때문이고, 그 목록을
  `ArticleReferenceTest.FollowUp`이 지킨다 — «푸는 것»만큼 «풀지 않는 것»을 본다.
...
손잡이는 `AssistantQueryPolicy`(`ssccops.assistant.query.*` — `top-k` **5**(#448에서 8에서
좁혔다 — 근거는 아래 «골든셋» 절의 hit@8 = hit@5) · `similarity-threshold` **0.35**(#457에서 0.5에서 내렸다 — 근거는 `application.yaml` 주석) · `-structured`/`-generic` 덮어쓰기 ·
`max-question-length` 1000 · `snippet-length` 200)다. 요청은 이 값들을 고르지 못한다 — 열면 «임계값 0으로 물어보기»가
...
### 인용 — 모델은 번호를 쓰고 조 표기는 서버가 붙인다 (#447)

프롬프트가 발췌마다 머리에 `--- 발췌 N ---`을 찍고, 모델은 **그 번호만** 대괄호에 넣는다 —
`[제7조 6항]`이 아니라 **`[3]`**이다. 조·쪽·문서명 문자열은 서버가 그 번호로 찾아 붙인다
(`RetrievedChunk.marker()` → `AssistantCitationResponse.marker`).

| | 옛 계약 `[제7조 6항]` | 지금 `[3]` |
|---|---|---|
| 모델이 없는 조를 지어낼 수 있나 | **예** → 답이 완성된 뒤 대조해야 했다 | **아니오** — 쓸 수 있는 값이 `1..N`뿐이다 |
| 판정 시점 | 답이 완성된 뒤 | **토큰이 닫히는 즉시** |
| 조·항 문자열을 누가 쓰나 | 모델 | **서버** |
| 스트리밍 | 불가(버릴 글자를 이미 보냈다) | 가능 |

**안전성이 내려간 것이 아니라 올라갔다** — 사후에 잡으려던 「틀린 조 번호」가 애초에 일어나지
않는다. 그래서 **번호 참조가 스트리밍의 전제이고 둘은 한 몸이다.**

- **`ref`가 본문의 `[3]`과 짝이다.** 배열 순서가 아니라 **발췌 번호**이며 다시 매기지 않는다 —
  다시 매기려면 이미 흘려보낸 본문을 고쳐 써야 한다. 화면은 `[3]`을 그대로 두거나 `marker`로
  갈아 그리며, **서버는 둘 다 내리고 고르지 않는다**(갈아 그리는 것은 다 받은 뒤에만 할 수 있다).
- **통과하지 못한 토큰은 본문에서도 지운다.** 인용 목록에서만 빼면 답변 문장에 `[9]`가 남아
  화면이 «근거가 있는 문장»으로 읽는다 — 인용 카드가 없다는 것을 알아채는 사람은 없다. 지우는
  것은 «출처를 달려다 실패한 것» 셋이다: **범위 밖 번호** · **옛 계약의 조·쪽 표기**
  (`[제7조]`·`[p.12]` — 이제 무엇도 통과하지 못한다) · **넣어 준 발췌의 문서명**. `[참고]` 같은
  평범한 대괄호는 건드리지 않는다. **토큰을 지울 때 앞의 공백도 함께 지운다** — 흘려보낸 뒤에는
  그 공백을 되돌릴 수 없어 «문장입니다 .»가 남는다.
- ⚠️ **`clause`는 언제나 `null`이 됐다.** 항 표기는 모델이 `[제7조 6항]`이라고 써 줄 때만 알 수
  있던 값인데, 대괄호에 번호만 들어오면서 «이 발췌의 어느 항인가»를 서버가 알 길이 없어졌다.
  지어내지 않는다 — 필드를 지우지 않은 것은 OpenAPI 하위 호환 게이트(#412)가 응답 필드 삭제를
  막기 때문이고, 항의 내용은 `snippet`이 그대로 보여 준다. 되살리려면 청크 메타에 «이 청크가
  담은 항 범위»가 있어야 한다.
...
- **같은 조가 두 청크로 갈려 있으면 카드가 둘이다.** 옛 계약에서는 조 표기가 같아 하나로 합쳐졌는데
  번호가 다르면 다른 발췌이고 `snippet`도 다르다 — 합치는 쪽이 오히려 정보를 버리는 것이었다.

### 흘려보내는 답 (SSE) — 봉투가 없는 유일한 자리 (#447)

`POST /v1/assistant/queries/stream`(`text/event-stream` · `SseEmitter`). 답 한 건이 실측
**7.5·9.3·12.2초**였고 그동안 화면이 비어 있었다 — 흘려보내면 같은 생성 시간에 첫 글자가
1~2초에 닿는다.

⚠️ **그 «1~2초»는 근거 없이 적힌 값이었고 실제로는 빗나가 있었다** (#453). 이정표를 달아 재
보니 첫 글자까지 **5.3초**였다 — `검색=904ms 첫수신=5272ms 첫송신=5276ms 소요=5732ms`
(2026-09-16 로컬 · 이력 0턴 · 발췌 5). 구성은 **검색 0.9초 + 생성 앞의 침묵 4.4초**이고, 답
전체가 흘러나오는 데는 **0.46초**다. **스트리밍은 처음부터 제대로 돌고 있었다** — 느린 것은
전송이 아니라 모델이 말하기 전 사고 시간이었고, 그 손잡이가 비어 있었다(루트 AGENTS.md ·
`GEMINI_CHAT_THINKING_LEVEL` — #453 이 `MINIMAL` 로 걸었고 **지금 기본값은 `MEDIUM`**이다, #463). `첫송신 - 첫수신 = 4ms`라 **아래 «조각 단위
판정»은 이 지연과 무관하다**(의심할 자리로 남겨 두지 말 것).

**`MINIMAL`을 건 뒤 같은 조건에서 `검색=814ms 첫수신=2674ms 첫송신=2676ms 소요=3347ms`** —
**첫 글자까지 5.3초 → 2.7초**이고 줄어든 것은 전부 사고 시간이다(4.37 → 1.86초). `인용=1
버린인용=0`이 그대로라 이 한 건에서는 품질 회귀가 없었다(표본이 하나씩이다). 남은 2.7초는
**검색 0.8초 + 모델 최소 왕복 1.9초**라 더 깎을 여지가 크지 않다 — 그다음 손댈 자리는 «첫
글자를 앞당기는 것»이 아니라 **검색을 `prepare`에서 떼어 응답을 먼저 여는 것**(첫 바이트가
ms 단위가 되고 «찾는 중»이 추측이 아니라 서버 신호가 된다)이다.

⚠️ **이 문단의 «2.7초»는 `MINIMAL`·`gemini-3.6-flash` 의 값이라 지금 것이 아니다** — 모델이
#461 에서 `gemini-3.5-flash-lite` 로, 사고 수준이 #463 에서 `MEDIUM` 으로 바뀌었고 후자는
**생성 구간을 2.5~3.3배로 되돌린 거래**다(인용 요청에서 `MINIMAL` 이 근거를 쥐고도 거절해서이며,
표는 루트 `AGENTS.md`에 있다). 남겨 두는 것은 **구성이 «검색 + 생성 앞의 침묵 + 흘려보내기»라는
것**이 모델·사고 수준과 무관하기 때문이다.

| 이벤트 | 본문 | 언제 |
|---|---|---|
| `delta` | `{text}` | 본문 조각. 이어 붙이면 `done`의 `answer`와 **글자 하나까지 같다** |
| `done` | `AssistantQueryResponse` 그대로 | 끝. 인용·판본·대화 식별자가 여기 온다 |
| `error` | `{code, message}` | **첫 바이트 뒤**의 실패. 이름이 `ApiResponse`의 오류와 같다 |

- ⚠️ **`ApiResponse` 봉투를 씌우지 않는다 — 전역 규약(`global/apipayload`)의 예외다.** 봉투는
  «요청 하나에 응답 하나»를 전제로 `success`·`code`·`message`를 매기는데 SSE 는 한 응답 안에서
  이벤트가 여러 번 나가므로, 씌우면 그 셋이 조각마다 되풀이될 뿐 아무것도 말하지 않는다. 대신
  **오류 이벤트만은 봉투와 같은 필드 이름**을 써 화면의 오류 처리를 두 벌로 만들지 않는다.
- ⚠️ **거절의 계단은 스트림이 열리기 전에 끝난다.** 404·413·503·403·429와 «근거 없음»은 모두
  `AssistantServiceImpl.prepare` 안에서 판정되고, `SseEmitter`는 그 뒤에야 돌아간다 — 그래서
  그 응답들이 **종전 그대로 상태 코드와 봉투**로 나간다. 순서를 흐트러뜨리면 «화면에 글자가
  나오다가 사실은 한도 초과였다»가 성립한다. 임계값 거절은 `delta` 없이 `done` 하나다.
- ⚠️ **흘려보낸 뒤에는 답을 버리지 않는다.** 모델이 출처를 하나도 달지 않았을 때 한 번에 받는
  경로는 답을 통째로 버리지만(3차 방어선), 여기서는 그 글자가 이미 읽혔다 — 다른 문장으로
  갈아치우는 것이 기각된 «사후 철회»다(`AssistantQueryResponse`의 «배지는 읽히지 않고 문장은
  읽힌다»와 같은 줄기인데, 이쪽은 **이미 보여 준 것을 회수하는 문제**라 답이 반대다). 그래서
  `done`이 `answered: false`에 **흘려보낸 문장 그대로**를 싣고(`ungrounded`) 화면이 «근거 없음»을
  표시한다. **대화에는 담지 않는다** — 우리가 뒤에 서지 않는 문장이다.
- ⚠️ **표식 거절은 그 반대편이다** (#455). `[근거없음]`은 답의 **첫 줄**에 오므로 화면에 글자가
  닿기 전에 판정이 끝나고, 그래서 임계값 거절과 같은 모양(`delta` 없이 `done` 하나)에 **정해진
  안내 문구**가 실린다 — 회수할 글자가 없으니 `ungrounded`가 아니다. 표식이 뒤늦게 오면(이미
  흘려보낸 뒤면) 그때는 `ungrounded`가 받는다: 같은 규칙의 두 얼굴이지 예외가 아니다.
- **본문 판정은 조각 단위로 돈다**(`CitationVerifier.Session`). 닫히지 않은 대괄호와 **그 앞의
  공백**을 붙들고 있다가 판정이 끝나면 내보낸다 — 버릴 토큰을 흘려보낸 뒤에 되돌릴 방법이 없기
  때문이다. 붙들기만 하지도 않는다: 줄이 바뀌거나 100자를 넘어 인용 토큰이 될 수 없다는 것이
  확정되면 평범한 글자로 내보낸다(그러지 않으면 `[`를 찍고 문장을 이어 가는 모델에서 화면이 멈춘다).
  **한 번에 받는 경로도 같은 세션을 쓴다** — 두 길이 글자 하나까지 같은 답을 내야 «스트리밍이 아닌
  경로의 회귀»라는 말이 성립한다.
- **구독을 우리 손으로 `boundedElastic`에 올린다.** Spring AI 1.1.8의
  `GoogleGenAiChatModel.internalStream`이 이미 그렇게 하고 있지만 **라이브러리 내부 결정이라 버전이
  오르며 바뀔 수 있고**, 그 줄이 사라지면 요청 스레드가 생성이 끝날 때까지 붙들려 `SseEmitter`가
  조각을 모았다 한 번에 내보낸다 — **화면에서는 스트리밍이 아니었던 것처럼 보이고** 아무도 원인을
  찾지 못한다(MCP 가 `immediateExecution`을 못 박아 둔 것과 같은 자리).
- **받는 쪽이 사라지면 생성을 멈춘다** — `delta`가 실패하면 `AssistantStreamClosedException`을
  던져 구독을 끊는다. 아무도 읽지 않는 답에 무료 쿼터를 쓰지 않기 위해서이고(§11), 그때는 오류
  이벤트도 로그 ERROR 도 남기지 않는다(알릴 상대가 없고, 공급자 장애로 세면 로그가 «탭을 닫은
  횟수»로 채워진다).
- **응답 상한은 새 손잡이가 아니라 `ssccops.assistant.gemini.call-timeout` + 10초다.** 같은 사실을
  두 벌 두면 «모델은 40초를 기다리는데 응답은 10초에 닫히는» 조합이 만들어지고, 그 고장은 «긴
  답변에서만 글자가 나오다 끊기는» 모양으로 나타난다.
- ⚠️ **OkHttp `callTimeout`은 본문을 다 읽을 때까지라 스트림도 상한에 걸린다 — 실측했다** (#448).
  상한 1,500ms에 흘려보내는 응답을 물리면 **조각 넷이 나간 뒤 1,863ms에 끊긴다.** 그때 실패의
  모양은 통째로 503이 아니라 **글자가 나오다 끊기고 `error` 이벤트가 오는 것**이다 —
  `GeminiClientConfigTest`가 그 성질을 붙들어 둔다. 상한이 40초가 된 것(#448)은 그래서 체감의
  문제가 아니라 «답이 끝까지 나올 수 있는가»의 문제다.
- **기존 경로를 갈아치우지 않았다.** ① 응답 유형을 바꾸는 것은 OpenAPI 하위 호환 게이트(#412)가
  막는 변경이고, ② 도구·스크립트가 «질문 하나에 JSON 하나»로 부를 자리가 남아야 하며(MCP 는 아직
  규정 도우미를 부르지 않지만 그 자리가 거기다 · #385), ③ 골든셋이 지표를 그 길로 잰다.
  **컨트롤러만 둘이고 그 아래는 한 벌이다.**
- **«검증기가 답을 버린 횟수»에 지표를 달지 않았다.** dev·prod 모두 Micrometer export 가 꺼져
  있어(루트 AGENTS.md «관측성») 그 수가 아무 데도 닿지 않는다 — 지금 읽을 수 있는 곳은 Kibana
  뿐이고, 사유가 로그 문장(`규정 도우미 거절 — … 사유=…`)에 그대로 실려 있어 셀 수 있다.
...
- **질문도 답변도 어디에도 저장하지 않는다**(질의 로그 표를 두지 않았다 · §9). **로그에도 싣지
  ...
  것은 «누가 · 몇 개의 근거로 · 답했는가 · **어느 구간에서** 얼마나 걸렸는가»다.
- 모델 호출 실패의 **원문을 응답에 싣지 않는다** — SDK의 예외 문장에 요청 본문이 섞여 나오고 그
  본문이 곧 사용자의 질문이다. 로그에는 남기며, **성공·실패·스트림 실패 셋이 같은 축을 쓴다**
  (`발췌` · `이력` · `소요`, #448) — 실패 줄에 `소요`가 없던 동안 «상한에 걸리고 있는가»는 요청
  줄과 오류 줄의 시각을 사람이 빼야 알 수 있었고, 그래서 #448이 한참 뒤에야 드러났다. 값을
  바꾼 뒤 다시 재는 일이 **로그 한 번 긁는 일**이어야 한다(이 기능에는 지표가 없다).
- **어디서 오래 걸렸는지는 이정표 넷이 말한다** (#453 · `AssistantServiceImpl.QueryTimeline`).
  `소요` 하나로는 «스트리밍인데 첫 글자까지 6초»의 정체를 가릴 수 없었다 — 검색이 느린 것과
  모델이 생각하는 것이 같은 한 숫자에 뭉쳐 있었다.

  | 칸 | 언제 | 빼면 나오는 구간 |
  |---|---|---|
  | `검색` | 검색이 끝났다 = **스트리밍에서 응답이 열리는 시각**(`SseEmitter` 반환) | **A** — DB · 질의 임베딩 왕복 · pgvector |
  | `첫수신` | 모델이 첫 조각을 줬다 | **B** = `첫수신 - 검색` — 연결 · 프롬프트 · 사고(thinking) |
  | `첫송신` | 화면으로 첫 `delta`가 나갔다 | 검증기가 붙든 시간 = `첫송신 - 첫수신` |
  | `소요` | 끝났다 (#448) | 생성 = `소요 - 첫수신` |

  **넷 다 «요청 시작으로부터 몇 ms»다 — 구간이 아니라 시각이다.** 구간으로 적으면 칸을 더할 때
  앞 칸의 뜻이 함께 바뀌어 옛 로그와 새 로그를 나란히 놓을 수 없다. **없는 이정표는 `-1`**이며
  (일괄 경로에는 조각이 없고, 검색 전에 끊긴 거절에는 `검색`이 없다) `0`을 쓰지 않는 것은 0ms 가
  실제로 나올 수 있는 값이기 때문이다(임베딩 캐시가 맞은 검색). 성공·거절·실패 세 줄이 모두 이
  축을 쓰며, **거절 줄이 A 를 가장 깨끗하게 보여 준다**(모델을 부르지 않으므로 B 가 없다).
  `AssistantServiceImplTest`가 순서(단조 증가)와 `-1`, 그리고 **본문이 실리지 않음**을 못 박는다.
...
- 값은 `ssccops.assistant.gemini.call-timeout`(**기본 `PT40S`** · #448)이고 **connect와 read를
  따로 줄 수 없다** — SDK가 받는 것이 전체 왕복 하나(`HttpOptions.timeout` → OkHttp
  `callTimeout`)라 기획안의 «connect 3s / read 20s»를 합쳤다.
- **20초에서 40초로 넓힌 근거는 실측이다** (#448 · 2026-09-16 로컬 · 문서 3건 67청크).
  **성공이 7~12초**라 20초는 여유가 8초뿐이었고, 조금만 느려지면 넘고 그 뒤에 아래 재시도가
  붙어 **35~43초를 기다린 끝에 503**이었다. 재시도를 좁혀 최악의 대기가 «상한 + 지연 한 번»이
  되었으므로 40초는 **오늘 실패에 이미 쓰고 있는 시간을 넘지 않으면서** 성공할 수 있는 창을 두
...
- ⚠️ **재시도의 정본은 `RetryTemplate`이 아니라 SDK다** (#448). `spring.ai.retry`가 만드는
  ...
  (`GenAiIOException` · `ApiException`)에는 **아무 일도 하지 않고**, 스트리밍 경로는 템플릿을
  아예 지나지 않는다. 대신 SDK가 `RetryInterceptor`를 기본값(5회 · 1s에서 2배씩 ·
  408·429·5xx·**모든 IOException**)으로 끼워 두는데, 그것이 OkHttp **애플리케이션 인터셉터**라
  `callTimeout`이 터진 뒤에도 잠든다 — 취소된 호출의 재요청은 서버에 닿지도 않으므로 **잠들기만
  한다.** 그것이 20초 상한을 35~43초 대기로 만든 자리다. 지금은 `GeminiClientConfig`가
  `spring.ai.retry`의 값을 그 인터셉터로 옮겨 담는다(2회 · 1s · 2배 · 최대 5s · **5xx만** ·
  `on-client-errors`가 408·429를 가른다). **색인 임베딩에는 아직 닿지 않는다** — 그쪽은 따로
  연결하므로 SDK 기본 재시도(429 포함 5회)를 그대로 쓴다.
...
- 지난 답변에 남아 있는 **번호는 이번 발췌와 아무 관계가 없다**(#447 — 발췌는 매 턴 새로 만든다).
  모델이 그것을 다시 쓰면 `CitationVerifier`가 엉뚱한 근거를 붙이거나(범위 안이면) 본문에서
  걷어낸다(범위 밖이면). 프롬프트 규칙 2에 «앞선 대화에서 썼던 번호는 다시 쓰지 않는다»를 적어 둔
  것은 **앞의 경우를 줄이려는 것이지 그 문장을 믿기 때문이 아니다.** 옛 계약(조 표기)에서는 이
  자리가 «없는 조를 인용한다»였고 3차 방어선이 잡아 냈는데, 번호 참조에서는 **범위 안의 엉뚱한
  근거**라 코드가 가려 낼 수 없다 — 잃은 것이 이것 하나다.

### 검색어는 언제나 이번 질문 하나다 — 넘어오는 것은 조 번호 하나뿐이다 (#465)

**대화는 답변 생성의 맥락이지 검색의 재료가 아니다.** 검색어에는 이번 질문의 글자만 들어간다.
...
**셋 다 여전히 기각이고, 넘어오는 것은 임베딩이 아니라 조 번호 하나다.** 옛 문단은 여기에
«이어 말한 질문이 홀로 서지 못하면 거절되며 그 거절이 안전한 실패다»라고 적어 두었는데,
**그 전제가 실측에서 무너졌다** — 「회칙 제3조를 인용해줘」 뒤의 「그 다음 조도 알려줘」는
사용자가 보기에 아무 모호함이 없는 질문이고, 제4조는 코퍼스에 있다. 거절은 안전한 실패가
아니라 **그냥 실패**였다.

고친 자리는 검색어가 아니라 **조 지목 핀**이다(질의 절의 「조 지목 핀」). 위 표가 걱정한
«두 주제 사이로 끌려가는 임베딩»은 검색어가 글자 하나 바뀌지 않으므로 일어나지 않는다.
...
| p95 질의 응답 시간 | < 5s (**우리 몫**) | **3 ms** (모델 왕복 제외 — 아래) |
| 발췌 폭 (#448) | hit@8 = hit@5 | **1.00 = 1.00** (10/10) |

🚦 **거절률 1.0이 기능 플래그를 켜는 조건이다.**

⚠️ **«질의 한 건»의 p95 목표는 5초가 아니라 15초다** (#448). 기획안 §10의 5초는 재기 전에 적은
값이고 실제 왕복은 7~12초다 — 그대로 두면 «목표는 언제나 빗나가 있다»가 정상이 된다. 위 표의
5초는 **Gemini 왕복을 뺀 우리 몫**의 목표이며 그쪽은 그대로다.

### 실측 벤치마크 — `./gradlew ragBench` (#465)

**골든셋이 잡지 못하는 것이 있다.** 스텁 임베딩(문자 n-gram)은 밀집 임베딩과 순위가 다르고,
시험지가 **단발 질의로만** 짜여 있어(그래야 지표가 질문 순서에 흔들리지 않는다) 이어 묻기가
아예 시험 범위 밖이다. 실제로 「그 다음 조」의 고장을 잡은 것은 골든셋이 아니라 화면이었다.

그래서 **로컬 DB에 적재된 코퍼스 + 실제 Gemini 임베딩**으로 도는 벤치마크를 따로 둔다 —
`geminiCheck`·`r2Check`와 같은 자리이고(실제 자격증명이 있어야 답이 나온다) `./gradlew test`에
섞지 않는다. 시험지는 `support/AssistantBenchmarkSet`이고 **대화 단위**다.

| 군 | 무엇을 | 왜 |
|---|---|---|
| A `article-pin` | 「제3조」·「부칙 제3조」·「제27조의2」 | #457의 핀이 살아 있는가 |
| B `follow-up` | 「그 다음 조」·「바로 앞 조」·「그 조 2항」 | #465가 고친 자리 |
| C `multi-article` | 「제18조와 제19조」 | 지목 둘이 함께 실리는가 |
| D `content` | 조 번호 없는 내용 질문 | 핀을 건드려 밀집 검색을 밀어내지 않았는가 |
| E `generic` | 평문 문서가 근거인 질문 | 조 단위가 아닌 청크도 서는가 |
| F `refusal` | 코퍼스 밖 주제 | 거절이 헐거워지지 않았는가 |

- **기본은 모델을 부르지 않는다.** 채팅 모델을 «발췌 번호를 전부 인용하는» 스텁으로 갈아
  `AssistantQueryResponse.citations`가 **모델에게 넣어 준 발췌 목록 그대로**가 되게 하고, 거기서
  재현율과 순위를 잰다. 시험지 한 바퀴가 28턴이라 채팅까지 부르면 무료 티어의 **하루치를 한 번에
  태운다**(#461의 일일 한도 실측). `--answer`로 실제 모델을 부르며, 그때만 F군이 판정된다 —
  스텁에는 거절의 주체가 없다.
- ⚠️ **스텁 설정 클래스에 `@Configuration`을 붙이지 말 것.** 이 도구가
  `org.sscc.ssccopsserver` 아래에 있어 컴포넌트 스캔이 집어 가고, 그러면 **`--answer`에서도
  스텁이 쓰인다**(실제로 한 번 그랬다 — 채팅 왕복이 없는데 `소요=859ms`였다). 애노테이션 없이
  `SpringApplicationBuilder`에 직접 넘기고, 모드와 배선이 갈리면 부팅을 멈춘다.
- **결과는 `build/rag-bench/{label}.tsv`**이고 `--compare`가 **바뀐 판정만** 찍는다. 고치기 전후를
  나란히 놓는 것이 이 도구의 쓸모다.
...
**2026-09-17 실측** (문서 3건 67청크 · `gemini-embedding-2` · 검색 모드):
...
| D 내용 | 5/6 | 5/6 |
| E 평문 | 2/2 | 2/2 |
| **합계** | **19/26 (73%)** | **25/26 (96%)** |

남은 한 건은 D3(「휴학하면 정회원 자격을 잃나요?」)이고 제7조는 오지만 제8조 8항이 안 온다 —
조 번호가 없는 질문이라 핀이 닿지 않는 자리이고, **밀집 검색만으로 두 조를 함께 끌어오는 문제**다.

⚠️ **로컬 코퍼스에 매인 시험지다.** 정답지가 «지금 적재돼 있는 문서 세 벌»을 전제하므로 코퍼스가
바뀌면 기대값도 함께 고친다. 그것이 골든셋과 합치지 않은 이유이기도 하다 — 그쪽은 테스트
리소스에 고정된 코퍼스를 쓴다.
...
인용 검증)이고, 모델 쪽 상한은 `ssccops.assistant.gemini.call-timeout`(기본 40초 · #448)이 건다.

**발췌 폭도 여기서 지킨다** (#448). `narrowingTheExcerptWidthToFiveLosesNoEvidence`가 hit@8과
hit@5를 나란히 재어 «속도를 재현율로 사지 않았다»를 숫자로 남긴다 — 6~8위의 근거가 답에
필요해지는 날(코퍼스가 자라거나 청킹이 바뀌는 날) 이 테스트가 먼저 떨어지고, 그때 되돌리는 것은
`application.yaml`의 `top-k`와 그 테스트의 `TOP_K` 둘이다.

### 스텁 모델은 프롬프트에서 발췌 번호를 읽는다

`GoldenChatModel`이 발췌 목록을 밖에서 받지 않고 **프롬프트의 `--- 발췌 N ---` 줄을 읽어** 상위
셋을 옮겨 쓴다 — 실제 모델이 보는 것이 그 문자열뿐이라, 프롬프트가 번호를 찍어 주지 않게 되는
순간 이 스텁도 인용하지 못한다. 「발췌마다 번호를 찍는다」가 인용 해석의 전제이므로
(`AssistantPrompt.user`) 그 전제가 깨지면 골든셋이 먼저 무너지는 것이 맞다.

날조하는 모델(**범위 밖 번호** `[99]`·`[98]`·`[97]`을 덧붙인다)과 인용을 하나도 달지 않는 모델도
같은 클래스가 흉내 낸다 — 앞은 «날조가 목록에서도 본문에서도 사라지고 맞는 인용은 남는가»,
뒤는 3차 방어선이다. **날조의 모양이 «없는 조»에서 «없는 발췌»로 좁아진 것**이 #447이 한 일이다.

### 인용 정확도가 재는 것이 달라졌다 (#447)

시험지의 정답지(`AssistantGoldenSet.ANSWERABLE`)는 여전히 `제7조`·`부칙 제3조` 같은 **표기
문자열**이다. 달라진 것은 **누가 그 문자열을 썼는가**다 — 예전에는 모델의 출력이었고 지금은
서버가 청크에서 만들어 응답에 싣는 값이다(`AssistantCitationResponse.marker`). 그래서 이 지표는
「모델이 조 번호를 옳게 옮겨 적었는가」가 아니라 **「검색이 올려 준 근거가 모델이 고른 번호를 거쳐
옳은 인용으로 오는가」**를 잰다. 앞의 물음은 **사라진 물음**이다.

**스트리밍 회귀도 같은 시험지로 본다** — `streamingGivesTheSameAnswersAsTheOneShotPath`가
답해야 하는 질문 전부를 흘려보내며 한 번 더 물어 답·인용·판본이 같은지 확인한다. 지표를 재는
것은 한 번에 받는 경로이므로, 여기가 어긋나면 그 지표가 화면이 보는 답을 더는 말하지 않는다.
...
- **MockMvc 로 SSE 를 보는 데 함정이 둘 있다** (#447 · `AssistantControllerTest`).
  ① **`isAsyncStarted()`가 내려가지 않는다** — `MockAsyncContext.complete()`가 그 값을 건드리지
  않아 실제 컨테이너와 다르다. 그래서 «마지막 이벤트가 실렸고 끝의 빈 줄까지 왔는가»로 기다린다
  (`SseEmitter`가 이벤트 하나를 여러 번에 나눠 쓰므로 중간에 읽으면 JSON 이 잘린다).
  ② **`getContentAsString()`이 ISO-8859-1로 읽는다** — `text/event-stream`에 charset 이 붙지
  않아서다. 한글이 깨진 채로 비교하면 원인을 찾는 데 한참 걸리므로 UTF-8을 명시한다.
- **스텁 모델의 `stream`은 답을 잘게 쪼갠다**(`StubChatModel` 3자 · 서비스 테스트의
  `RecordingChatModel`은 한 글자). 한 조각으로 주면 «토큰이 조각 경계에 걸쳐 온다»는 스트리밍의
  실제 조건을 한 번도 밟지 않는데, 인용 판정이 버텨야 하는 것이 바로 그 경계다. `ChatModel`의
  기본 `stream(Prompt)`은 **던진다**(`UnsupportedOperationException`) — 새 스텁을 만들 때 그
  메서드를 빠뜨리면 스트리밍 경로만 죽는다.
