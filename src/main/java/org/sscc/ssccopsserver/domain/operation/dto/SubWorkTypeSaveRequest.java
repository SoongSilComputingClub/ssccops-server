 ...
 * 되돌리면 안 된다.
 *
 * 승인자는 직위 enum(AuthorizerRole)이 아니라 결재 권한 코드 문자열이다 (#123). 어휘 검증
 * (AuthorityCode.subWorkApprovers 중 하나인가)을 역직렬화가 아니라 엔티티가 하는 것은, 승인
 * 정책 조합(승인 필요인데 승인자 없음, 정족수인데 인원 없음) 검증과 같은 자리여야 등록·수정
 * 두 경로가 같은 불변식을 지키기 때문이다. 승인이 필요 없을 때 남은 값을 '거절'이 아니라
 * '정리'하는 규칙도 엔티티에 함께 있다.
 */
public record SubWorkTypeSaveRequest(
        @NotBlank @Size(max = 100) String typeName,
        @NotNull Boolean approvalNeeded,
        /** 승인자 결재 권한 코드. 화면의 '-' 칩이 곧 미지정이라 null이 정상값이다 */
        @Size(max = 50) String authorizerAuthorityCode,
