 ...
 * 판단하는 자리가 두 곳(블록 유무·fileUrlAddr 유무)이 된다.
 *
 * **fileUrlAddr이 가리키는 오브젝트가 실제로 있다는 보장은 없다.** 참조 행은 업로드 허가를 발급할
 * 때 태어나고 서버는 PUT을 관측하지 않는다(#137, FileReferenceEntity 주석) — 화면은 이미지가
 * 깨지면 다시 올린다(재업로드가 UPSERT다).
 */
public record SessionFileReferenceResponse(Long fileReferenceId, String fileUrlAddr) {}
